...
func (d *KeyDatabase) DeleteDeviceKeys(ctx context.Context, userID string, deviceIDs []xtools.KeyID) error {
	return d.Writer.Do(d.DB, nil, func(txn *sql.Tx) error {
...
func (d *KeyDatabase) CrossSigningKeysForUser(ctx context.Context, userID string) (map[fclient.CrossSigningKeyPurpose]fclient.CrossSigningKey, error) {
	...
	for purpose, key := range keyMap {
		keyID := xtools.KeyID("ed25519:" + key.Encode())
		result := fclient.CrossSigningKey{
			UserID: userID,
			Usage:  []fclient.CrossSigningKeyPurpose{purpose},
			Keys: map[xtools.KeyID]spec.Base64Bytes{
				keyID: key,
			},
		}
		...
		for sigUserID, forSigUserID := range sigMap {
			...
			if result.Signatures == nil {
				result.Signatures = map[string]map[xtools.KeyID]spec.Base64Bytes{}
			}
			if _, ok := result.Signatures[sigUserID]; !ok {
				result.Signatures[sigUserID] = map[xtools.KeyID]spec.Base64Bytes{}
			}
...
func (d *KeyDatabase) CrossSigningSigsForTarget(ctx context.Context, originUserID, targetUserID string, targetKeyID xtools.KeyID) (types.CrossSigningSigMap, error) {
...
func (d *KeyDatabase) StoreCrossSigningSigsForTarget(
	ctx context.Context,
	originUserID string, originKeyID xtools.KeyID,
	targetUserID string, targetKeyID xtools.KeyID,
