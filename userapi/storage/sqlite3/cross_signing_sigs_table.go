...
import (
	...
	"github.com/withqb/coddy/userapi/storage/sqlite3/deltas"
...
func (s *crossSigningSigsStatements) SelectCrossSigningSigsForTarget(
	ctx context.Context, txn *sql.Tx, originUserID, targetUserID string, targetKeyID xtools.KeyID,
	...
	for rows.Next() {
		...
		if _, ok := r[userID]; !ok {
			r[userID] = map[xtools.KeyID]spec.Base64Bytes{}
		}
