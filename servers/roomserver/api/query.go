...
type QueryBulkStateContentResponse struct {
	...
	Rooms map[string]map[xtools.StateKeyTuple]string
}
...
func (r *QueryBulkStateContentResponse) UnmarshalJSON(data []byte) error {
	...
	r.Rooms = make(map[string]map[xtools.StateKeyTuple]string)
	for roomTuple, value := range wireFormat {
		fields := strings.Split(roomTuple, "\x1F")
		roomID := fields[0]
		if r.Rooms[roomID] == nil {
			r.Rooms[roomID] = make(map[xtools.StateKeyTuple]string)
		}
		r.Rooms[roomID][xtools.StateKeyTuple{
			EventType: fields[1],
			StateKey:  fields[2],
		}] = value
	}
...
func (rq *JoinRoomQuerier) CurrentStateEvent(ctx context.Context, roomID spec.RoomID, eventType string, stateKey string) (xtools.PDU, error) {
...
func (rq *JoinRoomQuerier) RestrictedRoomJoinInfo(ctx context.Context, roomID spec.RoomID, senderID spec.SenderID, localServerName spec.ServerName) (*xtools.RestrictedRoomJoinInfo, error) {
