...
import (
	...
	"github.com/withqb/coddy/servers/roomserver/storage/tables"
...
func (d *EventDatabase) Events(ctx context.Context, roomVersion xtools.RoomVersion, eventNIDs []types.EventNID) ([]types.Event, error) {
	return d.events(ctx, nil, roomVersion, eventNIDs)
}

func (d *EventDatabase) events(
	ctx context.Context, txn *sql.Tx, roomVersion xtools.RoomVersion, inputEventNIDs types.EventNIDs,
) ([]types.Event, error) {
...
func (d *Database) MembershipUpdater(
	...
	targetLocal bool, roomVersion xtools.RoomVersion,
...
func (d *Database) AssignRoomNID(ctx context.Context, roomID spec.RoomID, roomVersion xtools.RoomVersion) (roomNID types.RoomNID, err error) {
	// This should already be checked, let's check it anyway.
	_, err = xtools.GetRoomVersion(roomVersion)
	if err != nil {
		return 0, err
	}
	err = d.Writer.Do(d.DB, nil, func(txn *sql.Tx) error {
...
func (d *Database) GetOrCreateRoomInfo(ctx context.Context, event xtools.PDU) (roomInfo *types.RoomInfo, err error) {
	...
	// room.
	var roomVersion xtools.RoomVersion
...
func (d *Database) GetRoomVersion(ctx context.Context, roomID string) (xtools.RoomVersion, error) {
...
func (d *Database) assignRoomNID(
	ctx context.Context, txn *sql.Tx, roomID string, roomVersion xtools.RoomVersion,
) (types.RoomNID, error) {
...
func extractRoomVersionFromCreateEvent(event xtools.PDU) (
	xtools.RoomVersion, error,
) {
	var err error
	var roomVersion xtools.RoomVersion
	// Look for m.room.create events.
	if event.Type() != spec.MRoomCreate {
		return xtools.RoomVersion(""), nil
	}
	roomVersion = xtools.RoomVersionV1
	...
	if err = json.Unmarshal(event.Content(), &createContent); err != nil {
		return xtools.RoomVersion(""), err
	}
	// A room version was specified in the event content?
	if createContent.RoomVersion != nil {
		roomVersion = xtools.RoomVersion(*createContent.RoomVersion)
	}
...
func (d *EventDatabase) MaybeRedactEvent(
	ctx context.Context, roomInfo *types.RoomInfo, eventNID types.EventNID, event xtools.PDU, plResolver state.PowerLevelResolver,
...
func (d *EventDatabase) loadRedactionPair(
	ctx context.Context, txn *sql.Tx, roomInfo *types.RoomInfo, eventNID types.EventNID, event xtools.PDU,
...
func (d *Database) GetHistoryVisibilityState(ctx context.Context, roomInfo *types.RoomInfo, eventID string, domain string) ([]xtools.PDU, error) {
	...
	verImpl, err := xtools.GetRoomVersion(roomInfo.RoomVersion)
	if err != nil {
		return nil, err
	}
...
func (d *Database) GetBulkStateContent(ctx context.Context, roomIDs []string, tuples []xtools.StateKeyTuple, allowWildcards bool) ([]tables.StrippedEvent, error) {
	...
	var eventNIDs []types.EventNID
	eventNIDToVer := make(map[types.EventNID]xtools.RoomVersion)
	...
	for i := range events {
		...
		verImpl, err := xtools.GetRoomVersion(roomVer)
		if err != nil {
			return nil, err
		}
