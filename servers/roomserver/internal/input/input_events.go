...
func (r *Inputer) processRoomEvent(
	...
	if event.Version() == xtools.RoomVersionPseudoIDs && event.Type() == spec.MRoomMember {
...
func (r *Inputer) handleRemoteRoomUpgrade(ctx context.Context, event xtools.PDU) error {
...
func (r *Inputer) processStateBefore(
	...
	if rejectionErr = xtools.Allowed(event, &stateBeforeAuth, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		return r.Queryer.QueryUserIDForSender(ctx, roomID, senderID)
	}); rejectionErr != nil {
...
nextAuthEvent:
	for _, authEvent := range xtools.ReverseTopologicalOrdering(
		...
		if err := xtools.VerifyEventSignatures(ctx, authEvent, r.FSAPI.KeyRing(), func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return r.Queryer.QueryUserIDForSender(ctx, roomID, senderID)
		}); err != nil {
			continue nextAuthEvent
		}
		...
		err := xtools.Allowed(authEvent, auth, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return r.Queryer.QueryUserIDForSender(ctx, roomID, senderID)
		})
...
func (r *Inputer) kickGuests(ctx context.Context, event xtools.PDU, roomInfo *types.RoomInfo) error {
