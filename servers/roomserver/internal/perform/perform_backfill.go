...
import (
	...
	"github.com/withqb/coddy/servers/roomserver/auth"
...
// best effort.
func (r *Backfiller) fetchAndStoreMissingEvents(ctx context.Context, roomVer xtools.RoomVersion,
	...
	for _, srv := range servers {
		for id, ev := range missingMap {
			...
			loader := xtools.NewEventsLoader(roomVer, r.KeyRing, backfillRequester, backfillRequester.ProvideEvents, false)
			result, err := loader.LoadAndVerify(ctx, res.PDUs, xtools.TopologicalOrderByPrevEvents, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
				return r.Querier.QueryUserIDForSender(ctx, roomID, senderID)
			})
			if err != nil {
...
func (b *backfillRequester) StateIDsBeforeEvent(ctx context.Context, targetEvent xtools.PDU) ([]string, error) {
	...
	if len(targetEvent.PrevEventIDs()) == 0 && targetEvent.Type() == "m.room.create" && targetEvent.StateKeyEquals("") {
		xutil.GetLogger(ctx).WithField("room_id", targetEvent.RoomID()).Info("Backfilled to the beginning of the room")
...
func (b *backfillRequester) StateBeforeEvent(ctx context.Context, roomVer xtools.RoomVersion,
...
func (b *backfillRequester) ProvideEvents(roomVer xtools.RoomVersion, eventIDs []string) ([]xtools.PDU, error) {
...
func joinEventsFromHistoryVisibility(
	...
	if roomInfo == nil {
		return nil, xtools.HistoryVisibilityJoined, types.ErrorInvalidRoomInfo
	}
...
func persistEvents(ctx context.Context, db storage.Database, querier api.QuerySenderIDAPI, events []xtools.PDU) (types.RoomNID, map[string]types.Event) {
