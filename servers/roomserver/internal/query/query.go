...
import (
	...
	//"github.com/withqb/coddy/servers/roomserver/internal"
	...
	"github.com/withqb/coddy/servers/roomserver/acls"
	"github.com/withqb/coddy/servers/roomserver/api"
	"github.com/withqb/coddy/servers/roomserver/internal/helpers"
...
func (r *Queryer) RestrictedRoomJoinInfo(ctx context.Context, roomID spec.RoomID, senderID spec.SenderID, localServerName spec.ServerName) (*xtools.RestrictedRoomJoinInfo, error) {
	...
	if err = r.QueryServerJoinedToRoom(ctx, &req, &res); err != nil {
		xutil.GetLogger(ctx).WithError(err).Error("rsAPI.QueryServerJoinedToRoom failed")
	...
	if err != nil {
		xutil.GetLogger(ctx).WithError(err).Error("rsAPI.UserJoinedToRoom failed")
	...
	return &xtools.RestrictedRoomJoinInfo{
...
func (r *Queryer) QueryStateAfterEvents(
	...
	if len(request.PrevEventIDs) > 1 {
		...
		stateEvents, err = xtools.ResolveConflicts(
			info.RoomVersion, xtools.ToPDUs(stateEvents), xtools.ToPDUs(authEvents), func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
...
func (r *Queryer) loadStateAtEventIDs(ctx context.Context, roomInfo *types.RoomInfo, eventIDs []string) ([]xtools.PDU, bool, bool, error) {
...
func (r *Queryer) QueryRoomVersionForRoom(ctx context.Context, roomID string) (xtools.RoomVersion, error) {
...
func (r *Queryer) QueryBulkStateContent(ctx context.Context, req *api.QueryBulkStateContentRequest, res *api.QueryBulkStateContentResponse) error {
	...
	res.Rooms = make(map[string]map[xtools.StateKeyTuple]string)
	for _, ev := range events {
		if res.Rooms[ev.RoomID] == nil {
			res.Rooms[ev.RoomID] = make(map[xtools.StateKeyTuple]string)
		}
		room := res.Rooms[ev.RoomID]
		room[xtools.StateKeyTuple{
...
func (r *Queryer) CurrentStateEvent(ctx context.Context, roomID spec.RoomID, eventType string, stateKey string) (xtools.PDU, error) {
...
func (r *Queryer) LocallyJoinedUsers(ctx context.Context, roomVersion xtools.RoomVersion, roomNID types.RoomNID) ([]xtools.PDU, error) {
...
func (r *Queryer) QuerySenderIDForUser(ctx context.Context, roomID spec.RoomID, userID spec.UserID) (*spec.SenderID, error) {
	...
	switch version {
	case xtools.RoomVersionPseudoIDs:
