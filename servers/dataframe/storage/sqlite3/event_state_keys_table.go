...
const eventStateKeysSchema = `
	CREATE TABLE IF NOT EXISTS dataframe_event_state_keys (
    event_state_key_nid INTEGER PRIMARY KEY AUTOINCREMENT,
    event_state_key TEXT NOT NULL UNIQUE
	);
	INSERT INTO dataframe_event_state_keys (event_state_key_nid, event_state_key)
		VALUES (1, '')
...
const insertEventStateKeyNIDSQL = `
	INSERT INTO dataframe_event_state_keys (event_state_key) VALUES ($1)
	  ON CONFLICT DO NOTHING
	  RETURNING event_state_key_nid;
`

const selectEventStateKeyNIDSQL = `
	SELECT event_state_key_nid FROM dataframe_event_state_keys
...
const bulkSelectEventStateKeySQL = `
	SELECT event_state_key, event_state_key_nid FROM dataframe_event_state_keys
