...
const eventTypesSchema = `
  CREATE TABLE IF NOT EXISTS dataframe_event_types (
    event_type_nid INTEGER PRIMARY KEY AUTOINCREMENT,
    event_type TEXT NOT NULL UNIQUE
  );
  INSERT INTO dataframe_event_types (event_type_nid, event_type) VALUES
    (1, 'm.frame.create'),
    (2, 'm.frame.power_levels'),
    (3, 'm.frame.join_rules'),
    (4, 'm.frame.third_party_invite'),
    (5, 'm.frame.member'),
    (6, 'm.frame.redaction'),
    (7, 'm.frame.history_visibility') ON CONFLICT DO NOTHING;
`
...
const insertEventTypeNIDSQL = `
	INSERT INTO dataframe_event_types (event_type) VALUES ($1)
	  ON CONFLICT DO NOTHING
	  RETURNING event_type_nid;
`

const selectEventTypeNIDSQL = `
	SELECT event_type_nid FROM dataframe_event_types WHERE event_type = $1
`

// Bulk lookup from string event type to numeric ID for that event type.
// Takes an array of strings as the query parameter.
const bulkSelectEventTypeNIDSQL = `
	SELECT event_type, event_type_nid FROM dataframe_event_types
