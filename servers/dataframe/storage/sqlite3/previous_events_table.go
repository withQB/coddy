...
const previousEventSchema = `
  CREATE TABLE IF NOT EXISTS dataframe_previous_events (
...
// This should only be modified while holding a "FOR UPDATE" lock on the row in the frames table for this frame.
...
const insertPreviousEventSQL = `
	INSERT OR REPLACE INTO dataframe_previous_events
	  (previous_event_id, event_nids)
	  VALUES ($1, $2)
`

const selectPreviousEventNIDsSQL = `
	SELECT event_nids FROM dataframe_previous_events
	  WHERE previous_event_id = $1
`

// Check if the event is referenced by another event in the table.
// This should only be done while holding a "FOR UPDATE" lock on the row in the frames table for this frame.
const selectPreviousEventExistsSQL = `
	SELECT 1 FROM dataframe_previous_events
	  WHERE previous_event_id = $1
`
...
func CreatePrevEventsTable(db *sql.DB) error {
	...
	var cName string
	migrationName := "dataframe: drop column reference_sha from dataframe_prev_events"
	err = db.QueryRowContext(context.Background(), `SELECT p.name FROM sqlite_master AS m JOIN pragma_table_info(m.name) AS p WHERE m.name = 'dataframe_previous_events' AND p.name = 'previous_reference_sha256'`).Scan(&cName)
	if err != nil {
