...
type ServerACLs struct {
	acls      map[string]*serverACL // frame ID -> ACL
...
func (s *ServerACLs) OnServerACLUpdate(state xtools.PDU) {
	...
	}).Debugf("Updating server ACLs for %q", state.FrameID())
	s.aclsMutex.Lock()
	defer s.aclsMutex.Unlock()
	s.acls[state.FrameID()] = acls
}

func (s *ServerACLs) IsServerBannedFromFrame(serverName spec.ServerName, frameID string) bool {
	s.aclsMutex.RLock()
	// First of all check if we have an ACL for this frame. If we don't then
	// no servers are banned from the frame.
	acls, ok := s.acls[frameID]
	if !ok {
		s.aclsMutex.RUnlock()
		return false
	}
	...
	// the server is banned from the frame.
	...
	// the server is NOT banned from the frame.
