...
func (p *parsedRespState) Events() []xtools.PDU {
	eventsByID := make(map[string]xtools.PDU, len(p.AuthEvents)+len(p.StateEvents))
	...
	allEvents := make([]xtools.PDU, 0, len(eventsByID))
	for _, event := range eventsByID {
		allEvents = append(allEvents, event)
	}
	return xtools.ReverseTopologicalOrdering(
		xtools.ToPDUs(allEvents), xtools.TopologicalOrderByAuthEvents)
}
...
type missingStateReq struct {
	...
	federation      fedapi.DataframeFederationAPI
	framesMu         *internal.MutexByFrame
...
// request, as called from processFrameEvent.
// nolint:gocyclo
func (t *missingStateReq) processEventWithMissingState(
	ctx context.Context, e xtools.PDU, frameVersion xtools.FrameVersion,
	...
	// frame. There two ways that we can handle such a gap:
	//   1) We can fill in the gap using /get_missing_events
	//   2) We can leave the gap and request the state of the frame at
	...
	t.log = xutil.GetLogger(ctx).WithFields(map[string]interface{}{
		"txn_event":       e.EventID(),
		"frame_id":         e.FrameID(),
	...
	newEvents, isGapFilled, prevStatesKnown, err := t.getMissingEvents(ctx, e, frameVersion)
	if err != nil {
	...
	if isGapFilled {
		...
		for _, newEvent := range newEvents {
			err = t.inputer.processFrameEvent(ctx, t.virtualHost, &api.InputFrameEvent{
				Kind:         api.KindOld,
			...
			if err != nil {
				if _, ok := err.(types.RejectedError); !ok {
					return nil, fmt.Errorf("t.inputer.processFrameEvent (filling gap): %w", err)
				}
			}
		}
	}
	...
	resolvedState, err := t.lookupResolvedStateBeforeEvent(ctx, backwardsExtremity, frameVersion)
	if err != nil {
	...
	sendOutliers := func(resolvedState *parsedRespState) error {
		...
		outlierFrameEvents := make([]api.InputFrameEvent, 0, len(outliers))
		for _, outlier := range outliers {
			if hadEvents[outlier.EventID()] {
				continue
			}
			outlierFrameEvents = append(outlierFrameEvents, api.InputFrameEvent{
				Kind:   api.KindOutlier,
				Event:  &types.HeaderedEvent{PDU: outlier},
				Origin: t.origin,
			})
		}
		for _, ire := range outlierFrameEvents {
			if err = t.inputer.processFrameEvent(ctx, t.virtualHost, &ire); err != nil {
				if _, ok := err.(types.RejectedError); !ok {
					return fmt.Errorf("t.inputer.processFrameEvent (outlier): %w", err)
				}
			}
		}
		return nil
	...
	// Now send the backward extremity into the dataframe with the
	...
	if err != nil {
		if _, ok := err.(types.RejectedError); !ok {
			return nil, fmt.Errorf("t.inputer.processFrameEvent (backward extremity): %w", err)
		}
	}

	// Then send all of the newer backfilled events, of which will all be newer
	// than the backward extremity, into the dataframe without state. This way
	// they will automatically fast-forward based on the frame state at the
	...
	for _, newEvent := range newEvents {
		...
		if err != nil {
			if _, ok := err.(types.RejectedError); !ok {
				return nil, fmt.Errorf("t.inputer.processFrameEvent (fast forward): %w", err)
			}
		}
	}
	...
	resolvedState, err = t.lookupResolvedStateBeforeEvent(ctx, e, frameVersion)
	if err != nil {
...
func (t *missingStateReq) lookupResolvedStateBeforeEvent(ctx context.Context, e xtools.PDU, frameVersion xtools.FrameVersion) (*parsedRespState, error) {
	trace, ctx := internal.StartRegion(ctx, "lookupResolvedStateBeforeEvent")
	defer trace.EndRegion()

	type respState struct {
		// A snapshot is considered trustworthy if it came from our own dataframe.
	...
	// at this point we know we're going to have a gap: we need to work out the frame state at the new backwards extremity.
	...
	for _, prevEventID := range e.PrevEventIDs() {
		...
		// come from the dataframe, if we know all the required events, or it will
		// come from a remote server via /state_ids if not.
		prevState, trustworthy, err := t.lookupStateAfterEvent(ctx, frameVersion, e.FrameID(), prevEventID)
	...
	// frame if needed. This does a couple of things:
	...
	switch len(states) {
	case 0:
		extremityIsCreate := e.Type() == spec.MFrameCreate && e.StateKeyEquals("")
		if !extremityIsCreate {
			// There are no previous states and this isn't the beginning of the
			// frame - this is an error condition!
	...
	case 1:
		...
		isCreate := e.Type() == spec.MFrameCreate && e.StateKeyEquals("")
	...
	default:
		...
		var err error
		t.framesMu.Lock(e.FrameID())
		resolvedState, err = t.resolveStatesAndCheck(ctx, frameVersion, respStates, e)
		t.framesMu.Unlock(e.FrameID())
		switch err2 := err.(type) {
		case xtools.EventValidationError:
			if !err2.Persistable {
				return nil, err2
...
// lookupStateAfterEvent returns the frame state after `eventID`, which is the state before eventID with the state of `eventID` (if it's a state event)
// added into the mix.
func (t *missingStateReq) lookupStateAfterEvent(ctx context.Context, frameVersion xtools.FrameVersion, frameID, eventID string) (*parsedRespState, bool, error) {
	...
	var validationError error
	h, err := t.lookupEvent(ctx, frameVersion, frameID, eventID, false)
	switch e := err.(type) {
	case xtools.EventValidationError:
		if !e.Persistable {
...
func (t *missingStateReq) cacheAndReturn(ev xtools.PDU) xtools.PDU {
...
func (t *missingStateReq) lookupStateAfterEventLocally(ctx context.Context, eventID string) *parsedRespState {
	...
	frameState := state.NewStateResolution(t.db, t.frameInfo, t.inputer.Queryer)
	...
	stateEntries, err := frameState.LoadCombinedStateAfterEvents(ctx, stateAtEvents)
	if err != nil {
	...
	stateEvents, err := t.db.Events(ctx, t.frameInfo.FrameVersion, stateEventNIDs)
	if err != nil {
		t.log.WithError(err).Warnf("failed to load state events locally")
		return nil
	}
	res.StateEvents = make([]xtools.PDU, 0, len(stateEvents))
	...
	res.AuthEvents = make([]xtools.PDU, 0, len(stateEvents)*3)
	for _, ev := range stateEvents {
		t.haveEventsMutex.Lock()
	...
	// QueryStateAfterEvents does not return the auth events, so fetch them now. We know the dataframe has them else it wouldn't
	...
	if len(missingAuthEvents) > 0 {
		...
		events, err := t.db.EventsFromIDs(ctx, t.frameInfo, missingEventList)
		if err != nil {
			return nil
		}
...
// lookuptStateBeforeEvent returns the frame state before the event e, which is just /state_ids and/or /state depending on what
// the server supports.
func (t *missingStateReq) lookupStateBeforeEvent(ctx context.Context, frameVersion xtools.FrameVersion, frameID, eventID string) (
	*parsedRespState, error) {
	trace, ctx := internal.StartRegion(ctx, "lookupStateBeforeEvent")
	defer trace.EndRegion()

	// Attempt to fetch the missing state using /state_ids and /events
	return t.lookupMissingStateViaStateIDs(ctx, frameID, eventID, frameVersion)
}

func (t *missingStateReq) resolveStatesAndCheck(ctx context.Context, frameVersion xtools.FrameVersion, states []*parsedRespState, backwardsExtremity xtools.PDU) (*parsedRespState, error) {
	trace, ctx := internal.StartRegion(ctx, "resolveStatesAndCheck")
	defer trace.EndRegion()

	var authEventList []xtools.PDU
	var stateEventList []xtools.PDU
	for _, state := range states {
		authEventList = append(authEventList, state.AuthEvents...)
		stateEventList = append(stateEventList, state.StateEvents...)
	}
	resolvedStateEvents, err := xtools.ResolveConflicts(
		frameVersion, xtools.ToPDUs(stateEventList), xtools.ToPDUs(authEventList), func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
			return t.inputer.Queryer.QueryUserIDForSender(ctx, frameID, senderID)
		},
	)
	if err != nil {
		return nil, err
	...
	var validationError error
retryAllowedState:
	if err = checkAllowedByState(backwardsExtremity, resolvedStateEvents, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
		return t.inputer.Queryer.QueryUserIDForSender(ctx, frameID, senderID)
	}); err != nil {
		switch missing := err.(type) {
		case xtools.MissingAuthEventError:
			h, err2 := t.lookupEvent(ctx, frameVersion, backwardsExtremity.FrameID(), missing.AuthEventID, true)
			switch e := err2.(type) {
			case xtools.EventValidationError:
				if !e.Persistable {
					return nil, e
...
func (t *missingStateReq) getMissingEvents(ctx context.Context, e xtools.PDU, frameVersion xtools.FrameVersion) (newEvents []xtools.PDU, isGapFilled, prevStateKnown bool, err error) {
	trace, ctx := internal.StartRegion(ctx, "getMissingEvents")
	defer trace.EndRegion()

	logger := t.log.WithField("event_id", e.EventID()).WithField("frame_id", e.FrameID())
	latest, _, _, err := t.db.LatestEventIDs(ctx, t.frameInfo.FrameNID)
	if err != nil {
	...
	for _, server := range t.servers {
		...
		if m, err = t.federation.LookupMissingEvents(ctx, t.virtualHost, server, e.FrameID(), fclient.MissingEvents{
			Limit: 20,
			// The latest event IDs that the sender already has. These are skipped when retrieving the previous events of latest_events.
			EarliestEvents: latestEvents,
			// The event IDs to retrieve the previous events for.
			LatestEvents: []string{e.EventID()},
		}, frameVersion); err == nil {
			missingResp = &m
			break
		} else {
	...
	missingEvents := make([]xtools.PDU, 0, len(missingResp.Events))
	for _, ev := range missingResp.Events.UntrustedEvents(frameVersion) {
		if err = xtools.VerifyEventSignatures(ctx, ev, t.keys, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
			return t.inputer.Queryer.QueryUserIDForSender(ctx, frameID, senderID)
		}); err != nil {
			continue
		}
	...
	newEvents = xtools.ReverseTopologicalOrdering(
		xtools.ToPDUs(missingEvents), xtools.TopologicalOrderByPrevEvents)
...
Event:
	...
	// If we retrieved back to the beginning of the frame then there's nothing else
	// to do - we closed the gap.
	if len(earliestNewEvent.PrevEventIDs()) == 0 && earliestNewEvent.Type() == spec.MFrameCreate && earliestNewEvent.StateKeyEquals("") {
...
func (t *missingStateReq) isPrevStateKnown(ctx context.Context, e xtools.PDU) bool {
...
func (t *missingStateReq) lookupMissingStateViaState(
	ctx context.Context, frameID, eventID string, frameVersion xtools.FrameVersion,
) (respState *parsedRespState, err error) {
	trace, ctx := internal.StartRegion(ctx, "lookupMissingStateViaState")
	defer trace.EndRegion()

	state, err := t.federation.LookupState(ctx, t.virtualHost, t.origin, frameID, eventID, frameVersion)
	if err != nil {
		return nil, err
	}

	// Check that the returned state is valid.
	authEvents, stateEvents, err := xtools.CheckStateResponse(ctx, &fclient.RespState{
		StateEvents: state.GetStateEvents(),
		AuthEvents:  state.GetAuthEvents(),
	}, frameVersion, t.keys, nil, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
		return t.inputer.Queryer.QueryUserIDForSender(ctx, frameID, senderID)
	})
	if err != nil {
		return nil, err
...
func (t *missingStateReq) lookupMissingStateViaStateIDs(ctx context.Context, frameID, eventID string, frameVersion xtools.FrameVersion) (
	*parsedRespState, error) {
	...
	var stateIDs xtools.StateIDResponse
	var err error
	count := 0
	...
	for _, serverName := range t.servers {
		...
		stateIDs, err = t.federation.LookupStateIDs(reqctx, t.virtualHost, serverName, frameID, eventID)
		reqcancel()
		if err == nil {
			break
		}
	...
	if missingCount > concurrentRequests && missingCount > len(wantIDs)/2 {
		...
		return t.lookupMissingStateViaState(ctx, frameID, eventID, frameVersion)
	}
	...
	if missingCount > 0 {
		...
		fetch := func(missingEventID string) {
			h, herr := t.lookupEvent(ctx, frameVersion, frameID, missingEventID, false)
			switch e := herr.(type) {
			case xtools.EventValidationError:
				if !e.Persistable {
					return
...
func (t *missingStateReq) createRespStateFromStateIDs(
	stateIDs xtools.StateIDResponse,
...
func (t *missingStateReq) lookupEvent(ctx context.Context, frameVersion xtools.FrameVersion, _, missingEventID string, localFirst bool) (xtools.PDU, error) {
...
func checkAllowedByState(e xtools.PDU, stateEvents []xtools.PDU, userIDForSender spec.UserIDForSender) error {
	authUsingState := xtools.NewAuthEvents(nil)
	...
	return xtools.Allowed(e, &authUsingState, userIDForSender)
}
