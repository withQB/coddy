...
type StateResolution struct {
	...
	events   map[types.EventNID]xtools.PDU
	Querier  api.QuerySenderIDAPI
}

func NewStateResolution(db StateResolutionStorage, frameInfo *types.FrameInfo, querier api.QuerySenderIDAPI) StateResolution {
	return StateResolution{
		db:       db,
		frameInfo: frameInfo,
		events:   make(map[types.EventNID]xtools.PDU),
		Querier:  querier,
	}
}

type PowerLevelResolver interface {
	Resolve(ctx context.Context, eventID string) (*xtools.PowerLevelContent, error)
}

func (p *StateResolution) Resolve(ctx context.Context, eventID string) (*xtools.PowerLevelContent, error) {
	...
	events, err := p.db.Events(ctx, p.frameInfo.FrameVersion, []types.EventNID{plNID})
	if err != nil {
		return nil, err
	}
	if len(events) == 0 {
...
// LoadStateAtSnapshot loads the full state of a frame at a particular snapshot.
// This is typically the state before an event or the current state of a frame.
...
// LoadStateAtEvent loads the full state of a frame before a particular event.
...
func (v *StateResolution) LoadCombinedStateAfterEvents(
	...
	// the snapshot of the frame state before them was the same.
...
// This is used when we only want to load a subset of the frame state at a snapshot.
...
func (v *StateResolution) LoadStateAtSnapshotForStringTuples(
	...
	stateKeyTuples []xtools.StateKeyTuple,
) ([]types.StateEntry, error) {
...
func (v *StateResolution) stringTuplesToNumericTuples(
	ctx context.Context,
	stringTuples []xtools.StateKeyTuple,
	...
	eventTypes = xutil.UniqueStrings(eventTypes)
	eventTypeMap, err := v.db.EventTypeNIDs(ctx, eventTypes)
	if err != nil {
		return nil, err
	}
	stateKeys = xutil.UniqueStrings(stateKeys)
...
// This is used when we only want to load a subset of the frame state after a list of events.
// If there is no entry for a given event type and state key pair then it will be discarded.
...
var calculateStateDurations = prometheus.NewHistogramVec(
	...
	//    full_state_no_conflicts -> We created a new copy of the full frame state, but didn't enounter any conflicts
	//                               while doing so.
	//    full_state_with_conflicts -> We created a new copy of the full frame state and had to resolve conflicts to do so.
...
var calculateStateFullStateLength = prometheus.NewSummaryVec(
	prometheus.SummaryOpts{
		...
		Help:      "The length of the full frame state.",
	},
	[]string{"algorithm", "outcome"},
)
...
var calculateStateConflictLength = prometheus.NewSummaryVec(
	prometheus.SummaryOpts{
		...
		Help:      "The length of the conflicted frame state.",
	},
	[]string{"algorithm", "outcome"},
)
...
// CalculateAndStoreStateBeforeEvent calculates a snapshot of the state of a frame before an event.
...
// CalculateAndStoreStateAfterEvents finds the frame state after the given events.
...
func (v *StateResolution) CalculateAndStoreStateAfterEvents(
	...
	if len(prevStates) == 0 {
		...
		stateNID, err := v.db.AddState(ctx, v.frameInfo.FrameNID, nil, nil)
		if err != nil {
	...
	if len(prevStates) == 1 {
		...
		if len(stateBlockNIDs) < maxStateBlockNIDs {
			...
			stateNID, err := v.db.AddState(
				ctx, v.frameInfo.FrameNID, stateBlockNIDs, []types.StateEntry{prevState.StateEntry},
			)
			if err != nil {
				err = fmt.Errorf("v.db.AddState: %w", err)
			}
	...
	stateNID, err := v.calculateAndStoreStateAfterManyEvents(ctx, v.frameInfo.FrameNID, prevStates, metrics)
	if err != nil {
...
// maxStateBlockNIDs is the maximum number of state data blocks to use to encode a snapshot of frame state.
...
func (v *StateResolution) calculateAndStoreStateAfterManyEvents(
	...
	state, algorithm, conflictLength, err :=
		v.calculateStateAfterManyEvents(ctx, v.frameInfo.FrameVersion, prevStates)
	...
	return metrics.stop(v.db.AddState(ctx, frameNID, nil, state))
}

func (v *StateResolution) calculateStateAfterManyEvents(
	ctx context.Context, frameVersion xtools.FrameVersion,
	prevStates []types.StateAtEvent,
	...
	combined = combined[:xutil.SortAndUnique(stateEntrySorter(combined))]
	...
	if conflicts := findDuplicateStateKeys(combined); len(conflicts) > 0 {
		...
		resolved, err = v.resolveConflicts(ctx, frameVersion, notConflicted, conflicts)
		if err != nil {
...
func (v *StateResolution) resolveConflicts(
	ctx context.Context, version xtools.FrameVersion,
...
func (v *StateResolution) resolveConflictsV1(
	...
	needed := xtools.StateNeededForAuth(conflictedEvents)
	...
	resolvedEvents := xtools.ResolveStateConflicts(conflictedEvents, authEvents, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
		return v.Querier.QueryUserIDForSender(ctx, frameID, senderID)
	})
...
func (v *StateResolution) resolveConflictsV2(
	...
	authSets := make(map[string][]xtools.PDU, len(conflicted))
	authEvents := make([]xtools.PDU, 0, estimate*3)
	...
	if err = func() error {
		...
		for _, conflictedEvent := range conflictedEvents {
			...
			authSets[key], authEventMap, err = loader.loadAuthEvents(sctx, v.frameInfo, conflictedEvent, knownAuthEvents)
			if err != nil {
				return err
			}
	...
	// Resolve the conflicts.
	resolvedEvents := func() []xtools.PDU {
		resolvedTrace, _ := internal.StartRegion(ctx, "StateResolution.ResolveStateConflictsV2")
		defer resolvedTrace.EndRegion()

		return xtools.ResolveStateConflictsV2(
			conflictedEvents,
			nonConflictedEvents,
			authEvents,
			func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
				return v.Querier.QueryUserIDForSender(ctx, frameID, senderID)
			},
		)
	}()
...
func (v *StateResolution) stateKeyTuplesNeeded(stateKeyNIDMap map[string]types.EventStateKeyNID, stateNeeded xtools.StateNeeded) []types.StateKeyTuple {
...
// loadStateEvents loads the coddy events for a list of state entries.
...
func (v *StateResolution) loadStateEvents(
...
) ([]xtools.PDU, map[string]types.StateEntry, error) {
	trace, ctx := internal.StartRegion(ctx, "StateResolution.loadStateEvents")
	defer trace.EndRegion()

	result := make([]xtools.PDU, 0, len(entries))
	...
	if v.frameInfo == nil {
		return nil, nil, types.ErrorInvalidFrameInfo
	}
	events, err := v.db.Events(ctx, v.frameInfo.FrameVersion, eventNIDs)
	if err != nil {
		return nil, nil, err
	}
...
func (l *authEventLoader) loadAuthEvents(
	ctx context.Context, frameInfo *types.FrameInfo, event xtools.PDU, eventMap map[string]types.Event,
) ([]xtools.PDU, map[string]types.StateEntry, error) {
	l.Lock()
	defer l.Unlock()
	...
	for i := 0; i < len(queue); i++ {
		...
		if len(l.lookupFromDB) > 0 {
			eventsFromDB, err := l.v.db.EventsFromIDs(ctx, frameInfo, l.lookupFromDB)
			if err != nil {
	...
	nakedEvents := make([]xtools.PDU, 0, len(authEvents))
	for _, authEvent := range authEvents {
...
func UniqueStateSnapshotNIDs(nids []types.StateSnapshotNID) []types.StateSnapshotNID {
	return nids[:xutil.SortAndUnique(stateNIDSorter(nids))]
}
...
func uniqueStateBlockNIDs(nids []types.StateBlockNID) []types.StateBlockNID {
	return nids[:xutil.SortAndUnique(stateBlockNIDSorter(nids))]
}
