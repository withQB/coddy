...
import (
	...
	"github.com/withqb/coddy/apis/federationapi/storage/tables"
...
func (s *notaryServerKeysMetadataStatements) UpsertKey(
	ctx context.Context, txn *sql.Tx, serverName spec.ServerName, keyID xtools.KeyID, newNotaryID tables.NotaryID, newValidUntil spec.Timestamp,
...
func (s *notaryServerKeysMetadataStatements) SelectKeys(ctx context.Context, txn *sql.Tx, serverName spec.ServerName, keyIDs []xtools.KeyID) ([]xtools.ServerKeys, error) {
	var rows *sql.Rows
	var err error
	if len(keyIDs) == 0 {
