...
type FederationQueuePDUs interface {
	InsertQueuePDU(ctx context.Context, txn *sql.Tx, transactionID xtools.TransactionID, serverName spec.ServerName, nid int64) error
...
type FederationQueueTransactions interface {
	InsertQueueTransaction(ctx context.Context, txn *sql.Tx, transactionID xtools.TransactionID, serverName spec.ServerName, nid int64) error
...
type FederationNotaryServerKeysJSON interface {
	...
	InsertJSONResponse(ctx context.Context, txn *sql.Tx, keyQueryResponseJSON xtools.ServerKeys, serverName spec.ServerName, validUntil spec.Timestamp) (NotaryID, error)
}
...
type FederationNotaryServerKeysMetadata interface {
	...
	UpsertKey(ctx context.Context, txn *sql.Tx, serverName spec.ServerName, keyID xtools.KeyID, newNotaryID NotaryID, newValidUntil spec.Timestamp) (NotaryID, error)
	...
	SelectKeys(ctx context.Context, txn *sql.Tx, serverName spec.ServerName, keyIDs []xtools.KeyID) ([]xtools.ServerKeys, error)
...
type FederationServerSigningKeys interface {
	BulkSelectServerKeys(ctx context.Context, txn *sql.Tx, requests map[xtools.PublicKeyLookupRequest]spec.Timestamp) (map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult, error)
	UpsertServerKeys(ctx context.Context, txn *sql.Tx, request xtools.PublicKeyLookupRequest, key xtools.PublicKeyLookupResult) error
}
