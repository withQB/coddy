...
const outputFrameEventsTopologySchema = `
-- Stores output frame events received from the dataframe.
CREATE TABLE IF NOT EXISTS syncapi_output_frame_events_topology (
  event_id TEXT PRIMARY KEY,
  topological_position BIGINT NOT NULL,
  stream_position BIGINT NOT NULL,
  frame_id TEXT NOT NULL,

	UNIQUE(topological_position, frame_id, stream_position)
);
-- The topological order will be used in events selection and ordering
-- CREATE UNIQUE INDEX IF NOT EXISTS syncapi_event_topological_position_idx ON syncapi_output_frame_events_topology(topological_position, stream_position, frame_id);
`

const insertEventInTopologySQL = "" +
	"INSERT INTO syncapi_output_frame_events_topology (event_id, topological_position, frame_id, stream_position)" +
	" VALUES ($1, $2, $3, $4)" +
	" ON CONFLICT DO NOTHING"

const selectEventIDsInRangeASCSQL = "" +
	"SELECT event_id, topological_position, stream_position FROM syncapi_output_frame_events_topology" +
	" WHERE frame_id = $1 AND (" +
...
const selectPositionInTopologySQL = "" +
	"SELECT topological_position, stream_position FROM syncapi_output_frame_events_topology" +
	" WHERE event_id = $1"

const selectStreamToTopologicalPositionAscSQL = "" +
	"SELECT topological_position FROM syncapi_output_frame_events_topology WHERE frame_id = $1 AND stream_position >= $2 ORDER BY topological_position ASC LIMIT 1;"

const selectStreamToTopologicalPositionDescSQL = "" +
	"SELECT topological_position FROM syncapi_output_frame_events_topology WHERE frame_id = $1 AND stream_position <= $2 ORDER BY topological_position DESC LIMIT 1;"

const purgeEventsTopologySQL = "" +
	"DELETE FROM syncapi_output_frame_events_topology WHERE frame_id = $1"

type outputFrameEventsTopologyStatements struct {
	db                                        *sql.DB
...
func NewSqliteTopologyTable(db *sql.DB) (tables.Topology, error) {
	s := &outputFrameEventsTopologyStatements{
		db: db,
	}
	_, err := db.Exec(outputFrameEventsTopologySchema)
	if err != nil {
		return nil, err
	}
...
// insertEventInTopology inserts the given event in the frame's topology, based
// on the event's depth.
func (s *outputFrameEventsTopologyStatements) InsertEventInTopology(
	...
	_, err := sqlutil.TxStmt(txn, s.insertEventInTopologyStmt).ExecContext(
		ctx, event.EventID(), event.Depth(), event.FrameID(), pos,
	)
	return types.StreamPosition(event.Depth()), err
}

// SelectEventIDsInRange selects the IDs of events which positions are within a
// given range in a given frame's topological order. Returns the start/end topological tokens for
// the returned eventIDs.
// Returns an empty slice if no events match the given range.
func (s *outputFrameEventsTopologyStatements) SelectEventIDsInRange(
	...
	// Query the event IDs.
	rows, err := stmt.QueryContext(ctx, frameID, minDepth, maxDepth, maxDepth, maxStreamPos, limit)
...
// topology of the frame it belongs to.
func (s *outputFrameEventsTopologyStatements) SelectPositionInTopology(
...
// in the topology of the frame it belongs to from the given stream position.
func (s *outputFrameEventsTopologyStatements) SelectStreamToTopologicalPosition(
	ctx context.Context, txn *sql.Tx, frameID string, streamPos types.StreamPosition, backwardOrdering bool,
) (topoPos types.StreamPosition, err error) {
	if backwardOrdering {
		err = sqlutil.TxStmt(txn, s.selectStreamToTopologicalPositionDescStmt).QueryRowContext(ctx, frameID, streamPos).Scan(&topoPos)
	} else {
		err = sqlutil.TxStmt(txn, s.selectStreamToTopologicalPositionAscStmt).QueryRowContext(ctx, frameID, streamPos).Scan(&topoPos)
	}
	return
}

func (s *outputFrameEventsTopologyStatements) PurgeEventsTopology(
	ctx context.Context, txn *sql.Tx, frameID string,
) error {
	_, err := sqlutil.TxStmt(txn, s.purgeEventsTopologyStmt).ExecContext(ctx, frameID)
	return err
}
