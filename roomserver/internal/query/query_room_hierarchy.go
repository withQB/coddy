...
import (
	...
	fs "github.com/withqb/coddy/federationapi/api"
	roomserver "github.com/withqb/coddy/roomserver/api"
...
func (querier *Queryer) QueryNextRoomHierarchyPage(ctx context.Context, walker roomserver.RoomHierarchyWalker, limit int) ([]fclient.RoomHierarchyRoom, *roomserver.RoomHierarchyWalker, error) {
	...
	for len(unvisited) > 0 {
		...
		if create != nil {
			...
			if err != nil {
				xutil.GetLogger(ctx).WithError(err).WithField("create_content", create.Content()).Warn("failed to unmarshal m.room.create event")
			}
		...
		if !roomExists {
			...
			if err != nil {
				xutil.GetLogger(ctx).WithError(err).WithField("room_id", queuedRoom.RoomID).Error("failed to extract references for room")
				continue
			}
		...
		for i := len(discoveredChildEvents) - 1; i >= 0; i-- {
			...
			if err != nil {
				xutil.GetLogger(ctx).WithError(err).WithField("invalid_room_id", ev.StateKey).WithField("parent_room_id", queuedRoom.RoomID).Warn("Invalid room ID in m.space.child state event")
			} else {
...
func authorisedServer(ctx context.Context, querier *Queryer, roomID spec.RoomID, callerServerName spec.ServerName) bool {
	...
	if joinRuleEv != nil {
		...
		if ruleErr != nil {
			xutil.GetLogger(ctx).WithError(ruleErr).WithField("parent_room_id", roomID).Warn("failed to get join rule")
			return false
		}
	...
	for _, allowedRoomID := range allowJoinedToRoomIDs {
		...
		if err != nil {
			xutil.GetLogger(ctx).WithError(err).Error("failed to QueryJoinedHostServerNamesInRoom")
			continue
		}
...
func authorisedUser(ctx context.Context, querier *Queryer, clientCaller *userapi.Device, roomID spec.RoomID, parentRoomID *spec.RoomID) (authed bool, isJoinedOrInvited bool) {
	...
	roomMemberTuple := xtools.StateKeyTuple{
	...
	if parentRoomID != nil && joinRuleEv != nil {
		...
		rule, ruleErr := joinRuleEv.JoinRule()
		if ruleErr != nil {
			xutil.GetLogger(ctx).WithError(ruleErr).WithField("parent_room_id", parentRoomID).Warn("failed to get join rule")
		...
		if allowed {
			...
			if err != nil {
				xutil.GetLogger(ctx).WithError(err).WithField("parent_room_id", parentRoomID).Warn("failed to check user is joined to parent room")
			} else {
...
func roomExists(ctx context.Context, querier *Queryer, roomID spec.RoomID) bool {
	...
	if err != nil {
		xutil.GetLogger(ctx).WithError(err).Error("failed to QueryServerJoinedToRoom")
		return false
	}
...
func federatedRoomInfo(ctx context.Context, querier *Queryer, caller types.DeviceOrServerName, suggestedOnly bool, roomID spec.RoomID, vias []string) *fclient.RoomHierarchyResponse {
	...
	if ok {
		xutil.GetLogger(ctx).Debugf("Returning cached response for %s", roomID)
		return &resp
	}
	xutil.GetLogger(ctx).Debugf("Querying %s via %+v", roomID, vias)
	...
	for _, serverName := range vias {
		...
		if err != nil {
			xutil.GetLogger(ctx).WithError(err).Warnf("failed to call RoomHierarchies on server %s", serverName)
			continue
		}
...
func publicRoomsChunk(ctx context.Context, querier *Queryer, roomID spec.RoomID) *fclient.PublicRoom {
	...
	if err != nil {
		xutil.GetLogger(ctx).WithError(err).Error("failed to PopulatePublicRooms")
		return nil
	}
	if len(pubRooms) == 0 {
		return nil
...
func stripped(ev xtools.PDU) *fclient.RoomHierarchyStrippedEvent {
	if ev.StateKey() == nil {
		return nil
	}
...
func restrictedJoinRuleAllowedRooms(ctx context.Context, joinRuleEv *types.HeaderedEvent) (allows []spec.RoomID) {
	...
	if err := json.Unmarshal(joinRuleEv.Content(), &jrContent); err != nil {
		xutil.GetLogger(ctx).Warnf("failed to check join_rule on room %s: %s", joinRuleEv.RoomID(), err)
		return nil
	}
	for _, allow := range jrContent.Allow {
		if allow.Type == spec.MRoomMembership {
			...
			if err != nil {
				xutil.GetLogger(ctx).Warnf("invalid room ID '%s' found in join_rule on room %s: %s", allow.RoomID, joinRuleEv.RoomID(), err)
			} else {
