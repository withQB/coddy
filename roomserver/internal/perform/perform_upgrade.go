...
func (r *Upgrader) PerformRoomUpgrade(
	ctx context.Context,
	roomID string, userID spec.UserID, roomVersion xtools.RoomVersion,
...
func (r *Upgrader) performRoomUpgrade(
	...
	if err != nil {
		...
		xutil.GetLogger(ctx).WithField("userID", userID).WithField("roomID", *fullRoomID).Error("No senderID for user")
	...
	newRoomID := fmt.Sprintf("!%s:%s", xutil.RandomString(16), userID.Domain())
...
func (r *Upgrader) getRoomPowerLevels(ctx context.Context, roomID string) (*xtools.PowerLevelContent, error) {
	oldPowerLevelsEvent := api.GetStateEvent(ctx, r.URSAPI, roomID, xtools.StateKeyTuple{
...
func (r *Upgrader) restrictOldRoomPowerLevels(ctx context.Context, evTime time.Time, senderID spec.SenderID, userDomain spec.ServerName, roomID string) error {
	...
	restrictedPowerLevelsHeadered, resErr := r.makeHeaderedEvent(ctx, evTime, senderID, userDomain, roomID, xtools.FledglingEvent{
	...
	case api.ErrNotAllowed:
		xutil.GetLogger(ctx).WithField(logrus.ErrorKey, resErr).Warn("UpgradeRoom: Could not restrict power levels in old room")
	case nil:
...
func (r *Upgrader) clearOldCanonicalAliasEvent(ctx context.Context, oldRoom *api.QueryLatestEventsAndStateResponse, evTime time.Time, senderID spec.SenderID, userDomain spec.ServerName, roomID string) error {
	...
	emptyCanonicalAliasEvent, resErr := r.makeHeaderedEvent(ctx, evTime, senderID, userDomain, roomID, xtools.FledglingEvent{
		Type:    spec.MRoomCanonicalAlias,
		Content: map[string]interface{}{},
	})
	switch resErr.(type) {
	case api.ErrNotAllowed:
		xutil.GetLogger(ctx).WithField(logrus.ErrorKey, resErr).Warn("UpgradeRoom: Could not set empty canonical alias event in old room")
	case nil:
...
func publishNewRoomAndUnpublishOldRoom(
	...
	if err := URSAPI.PerformPublish(ctx, &api.PerformPublishRequest{
		...
		// treat as non-fatal since the room is already made by this point
		xutil.GetLogger(ctx).WithError(err).Error("failed to un-publish room")
	}
}
...
func (r *Upgrader) userIsAuthorized(ctx context.Context, senderID spec.SenderID, roomID string,
	...
	plEvent := api.GetStateEvent(ctx, r.URSAPI, roomID, xtools.StateKeyTuple{
		EventType: spec.MRoomPowerLevels,
		StateKey:  "",
	})
	if plEvent == nil {
...
// nolint:gocyclo
func (r *Upgrader) generateInitialEvents(ctx context.Context, oldRoom *api.QueryLatestEventsAndStateResponse, senderID spec.SenderID, roomID string, newVersion xtools.RoomVersion, tombstoneEvent *types.HeaderedEvent) ([]xtools.FledglingEvent, error) {
	state := make(map[xtools.StateKeyTuple]*types.HeaderedEvent, len(oldRoom.StateEvents))
	for _, event := range oldRoom.StateEvents {
	...
	oldCreateEvent := state[xtools.StateKeyTuple{EventType: spec.MRoomCreate, StateKey: ""}]
	oldMembershipEvent := state[xtools.StateKeyTuple{EventType: spec.MRoomMember, StateKey: string(senderID)}]
	oldPowerLevelsEvent := state[xtools.StateKeyTuple{EventType: spec.MRoomPowerLevels, StateKey: ""}]
	oldJoinRulesEvent := state[xtools.StateKeyTuple{EventType: spec.MRoomJoinRules, StateKey: ""}]
	...
	newCreateContent["predecessor"] = xtools.PreviousRoom{
	...
	if _, ok := state[xtools.StateKeyTuple{EventType: spec.MRoomGuestAccess, StateKey: ""}]; !ok {
...
func (r *Upgrader) sendInitialEvents(ctx context.Context, evTime time.Time, senderID spec.SenderID, userDomain spec.ServerName, newRoomID string, newVersion xtools.RoomVersion, eventsToMake []xtools.FledglingEvent) error {
	var err error
	...
	for i, e := range eventsToMake {
		...
		var verImpl xtools.IRoomVersion
		verImpl, err = xtools.GetRoomVersion(newVersion)
		if err != nil {
			return err
		}
		...
		if err = xtools.Allowed(event, &authEvents, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
...
func (r *Upgrader) makeHeaderedEvent(ctx context.Context, evTime time.Time, senderID spec.SenderID, senderDomain spec.ServerName, roomID string, event xtools.FledglingEvent) (*types.HeaderedEvent, error) {
	proto := xtools.ProtoEvent{
		SenderID: string(senderID),
		RoomID:   roomID,
		Type:     event.Type,
	...
	if err = xtools.Allowed(headeredEvent.PDU, &provider, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		return r.URSAPI.QueryUserIDForSender(ctx, roomID, senderID)
	}); err != nil {
