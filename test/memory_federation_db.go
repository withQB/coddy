...
import (
	...
	"github.com/withqb/coddy/federationapi/storage/shared/receipt"
...
type InMemoryFederationDatabase struct {
	...
	pendingEDUs        map[*receipt.Receipt]*xtools.EDU
...
func NewInMemoryFederationDatabase() *InMemoryFederationDatabase {
	return &InMemoryFederationDatabase{
		...
		pendingEDUs:        make(map[*receipt.Receipt]*xtools.EDU),
...
func (d *InMemoryFederationDatabase) GetPendingEDUs(
	...
	serverName spec.ServerName,
	limit int,
) (edus map[*receipt.Receipt]*xtools.EDU, err error) {
	d.dbMutex.Lock()
	defer d.dbMutex.Unlock()

	eduCount := 0
	edus = make(map[*receipt.Receipt]*xtools.EDU)
...
func (d *InMemoryFederationDatabase) FetchKeys(ctx context.Context, requests map[xtools.PublicKeyLookupRequest]spec.Timestamp) (map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult, error) {
	return nil, nil
}
...
func (d *InMemoryFederationDatabase) StoreKeys(ctx context.Context, results map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult) error {
	return nil
}
...
func (d *InMemoryFederationDatabase) UpdateNotaryKeys(ctx context.Context, serverName spec.ServerName, serverKeys xtools.ServerKeys) error {
	return nil
}

func (d *InMemoryFederationDatabase) GetNotaryKeys(ctx context.Context, serverName spec.ServerName, optKeyIDs []xtools.KeyID) ([]xtools.ServerKeys, error) {
	return nil, nil
}
