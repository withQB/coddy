...
func OnIncomingMessagesRequest(
	...
	if err != nil {
		xutil.GetLogger(req.Context()).WithError(err).Error("mreq.retrieveEvents failed")
		return xutil.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
...
func (r *messagesReq) retrieveEvents(ctx context.Context, rsAPI api.SyncRoomserverAPI) (
	...
	xutil.GetLogger(r.ctx).WithFields(logrus.Fields{
		"start":     r.from,
		"end":       r.to,
...
func (r *messagesReq) backfill(roomID string, backwardsExtremities map[string][]string, limit int) ([]*rstypes.HeaderedEvent, error) {
	...
	if res.HistoryVisibility == "" {
		res.HistoryVisibility = xtools.HistoryVisibilityShared
	}
	events := res.Events
	for i := range events {
