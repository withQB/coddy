...
import (
	...
	"github.com/withqb/coddy/syncapi/notifier"
...
func (p *PDUStreamProvider) addRoomDeltaToResponse(
	...
	for i, ev := range events {
		if ev.Version() != xtools.RoomVersionPseudoIDs {
			continue
		}
	...
	for i := range sEvents {
		...
		if ev.Version() == xtools.RoomVersionPseudoIDs && ev.Type() == spec.MRoomPowerLevels && ev.StateKeyEquals("") {
			var newEvent xtools.PDU
	...
	case spec.Join:
		...
		jr.Timeline.Events = synctypes.ToClientEvents(xtools.ToPDUs(events), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		...
		jr.State.Events = synctypes.ToClientEvents(xtools.ToPDUs(delta.StateEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
	...
	case spec.Peek:
		...
		jr.Timeline.Events = synctypes.ToClientEvents(xtools.ToPDUs(recentEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
	...
	case spec.Ban:
		...
		lr.Timeline.Events = synctypes.ToClientEvents(xtools.ToPDUs(events), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
		// If we are limited by the filter AND the history visibility filter
		// didn't "remove" events, return that the response is limited.
		lr.Timeline.Limited = limited && len(events) == len(recentEvents)
		lr.State.Events = synctypes.ToClientEvents(xtools.ToPDUs(delta.StateEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
		})
...
func (p *PDUStreamProvider) updatePowerLevelEvent(ctx context.Context, ev *rstypes.HeaderedEvent, eventFormat synctypes.ClientEventFormat) (xtools.PDU, error) {
	...
	if !prevContent.Exists() {
		...
		evNew, err = xtools.MustGetRoomVersion(ev.Version()).NewEventFromTrustedJSON(newEv, false)
		if err != nil {
			return nil, err
		}
	...
	var evNew xtools.PDU
	evNew, err = xtools.MustGetRoomVersion(ev.Version()).NewEventFromTrustedJSONWithEventID(ev.EventID(), newEv, false)
	if err != nil {
		return nil, err
	}
...
func (p *PDUStreamProvider) getJoinResponseForCompleteSync(
	...
	jr.State.Events = synctypes.ToClientEvents(xtools.ToPDUs(stateEvents), eventFormat, func(roomID spec.RoomID, senderID spec.SenderID) (*spec.UserID, error) {
		return p.rsAPI.QueryUserIDForSender(ctx, roomID, senderID)
	})
	return jr, nil
}
