...
import (
	...
	"github.com/withqb/coddy/syncapi/streams"
...
func (rp *RequestPool) OnIncomingSyncRequest(req *http.Request, device *userapi.Device) xutil.JSONResponse {
	...
	for {
		...
		if !rp.shouldReturnImmediately(syncReq, currentPos) {
			...
			giveup := func() xutil.JSONResponse {
				...
				// https://github.com/withqb/synapse/blob/29f06704b8871a44926f7c99e73cf4a978fb8e81/synapse/rest/client/sync.py#L276-L281
...
func (rp *RequestPool) OnIncomingKeyChangeRequest(req *http.Request, device *userapi.Device) xutil.JSONResponse {
	...
	syncReq, err := newSyncRequest(req, *device, rp.db)
	if err != nil {
		xutil.GetLogger(req.Context()).WithError(err).Error("newSyncRequest failed")
		return xutil.JSONResponse{
