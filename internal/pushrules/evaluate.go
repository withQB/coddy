...
func ruleMatches(rule *Rule, kind Kind, event xtools.PDU, ec EvaluationContext, userIDForSender spec.UserIDForSender) (bool, error) {
	if !rule.Enabled {
		return false, nil
	}
...
func conditionMatches(cond *Condition, event xtools.PDU, ec EvaluationContext) (bool, error) {
	switch cond.Kind {
...
func patternMatches(key, pattern string, event xtools.PDU) (bool, error) {
