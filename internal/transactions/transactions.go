...
type txnsMap map[CacheKey]*xutil.JSONResponse
...
func (t *Cache) FetchTransaction(accessToken, txnID string, u *url.URL) (*xutil.JSONResponse, bool) {
	t.RLock()
	defer t.RUnlock()
...
func (t *Cache) AddTransaction(accessToken, txnID string, u *url.URL, res *xutil.JSONResponse) {
	t.Lock()
	defer t.Unlock()
