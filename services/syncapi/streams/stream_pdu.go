...
// The max number of per-frame goroutines to have running.
...
func (p *PDUStreamProvider) CompleteSync(
	...
	// backwards, so that we show the most recent events in the frame.
	r := types.Range{
		From:      to,
		To:        0,
		Backwards: true,
	...
	// Extract frame state and recent events for all frames the user is joined to.
	joinedFrameIDs, err := snapshot.FrameIDsWithMembership(ctx, req.Device.UserID, spec.Join)
	if err != nil {
		req.Log.WithError(err).Error("p.DB.FrameIDsWithMembership failed")
		return from
	}

	stateFilter := req.Filter.Frame.State
	eventFilter := req.Filter.Frame.Timeline
	...
	recentEvents, err := snapshot.RecentEvents(ctx, joinedFrameIDs, r, &eventFilter, true, true)
	if err != nil {
		return from
	}
	// Build up a /sync response. Add joined frames.
	for _, frameID := range joinedFrameIDs {
		...
		// get the join response for each frame
		jr, jerr := p.getJoinResponseForCompleteSync(
			ctx, snapshot, frameID, &stateFilter, req.WantFullState, req.Device, false,
	...
	if len(peeks) > 0 {
		peekFrames := make([]string, 0, len(peeks))
		for _, peek := range peeks {
			if !peek.Deleted {
				peekFrames = append(peekFrames, peek.FrameID)
			}
		}

		recentEvents, err = snapshot.RecentEvents(ctx, peekFrames, r, &eventFilter, true, true)
		if err != nil {
			return from
		}

		for _, frameID := range peekFrames {
			var jr *types.JoinResponse
			events := recentEvents[frameID]
			jr, err = p.getJoinResponseForCompleteSync(
				ctx, snapshot, frameID, &stateFilter, req.WantFullState, req.Device, true,
				events.Events, events.Limited, eventFormat,
			)
			if err != nil {
			...
			req.Response.Frames.Peek[frameID] = jr
		}
	}
...
func (p *PDUStreamProvider) IncrementalSync(
	...
	var syncJoinedFrames []string

	stateFilter := req.Filter.Frame.State
	eventFilter := req.Filter.Frame.Timeline

	if req.WantFullState {
		if stateDeltas, syncJoinedFrames, err = snapshot.GetStateDeltasForFullStateSync(ctx, req.Device, r, req.Device.UserID, &stateFilter, p.rsAPI); err != nil {
			req.Log.WithError(err).Error("p.DB.GetStateDeltasForFullStateSync failed")
			return from
		}
	} else {
		if stateDeltas, syncJoinedFrames, err = snapshot.GetStateDeltas(ctx, req.Device, r, req.Device.UserID, &stateFilter, p.rsAPI); err != nil {
			req.Log.WithError(err).Error("p.DB.GetStateDeltas failed")
			return from
		}
	}

	for _, frameID := range syncJoinedFrames {
		req.Frames[frameID] = spec.Join
	}
	...
	for _, delta := range stateDeltas {
		newRange := r
		// If this frame was joined in this sync, try to fetch
		...
		if pos, err = p.addFrameDeltaToResponse(ctx, snapshot, req.Device, newRange, delta, &eventFilter, &stateFilter, req); err != nil {
			req.Log.WithError(err).Error("d.addFrameDeltaToResponse failed")
			if err == context.DeadlineExceeded || err == context.Canceled || err == sql.ErrTxDone {
				return newPos
			}
			continue
		}
		// Reset the position, as it is only for the special case of newly joined frames
		if delta.NewlyJoined {
			pos = newRange.From
		}
		switch {
...
// nolint:gocyclo
func (p *PDUStreamProvider) addFrameDeltaToResponse(
	ctx context.Context,
	snapshot storage.DatabaseTransaction,
	device *userapi.Device,
	r types.Range,
	delta types.StateDelta,
	eventFilter *synctypes.FrameEventFilter,
	...
	recentStreamEvents := dbEvents[delta.FrameID].Events
	limited := dbEvents[delta.FrameID].Limited

	recEvents := xtools.ReverseTopologicalOrdering(
		xtools.ToPDUs(snapshot.StreamEventsToEvents(ctx, device, recentStreamEvents, p.rsAPI)),
	...
	if stateFilter.LazyLoadMembers {
		delta.StateEvents, err = p.lazyLoadMembers(
			ctx, snapshot, delta.FrameID, true, limited, stateFilter,
	...
	for _, recentEvent := range recentStreamEvents {
		if recentEvent.Type() == spec.MFrameMember && recentEvent.StateKey() != nil {
	...
	events, err := applyHistoryVisibilityFilter(ctx, snapshot, p.rsAPI, delta.FrameID, device.UserID, recentEvents)
	if err != nil {
	...
	for i, ev := range events {
		if ev.Version() != xtools.FrameVersionPseudoIDs {
			continue
		}
		if ev.Type() != spec.MFramePowerLevels || !ev.StateKeyEquals("") {
			continue
		}
		var newEvent xtools.PDU
	...
	sEvents := xtools.HeaderedReverseTopologicalOrdering(
		xtools.ToPDUs(removeDuplicates(delta.StateEvents, events)),
	...
	for i := range sEvents {
		...
		if ev.Version() == xtools.FrameVersionPseudoIDs && ev.Type() == spec.MFramePowerLevels && ev.StateKeyEquals("") {
			var newEvent xtools.PDU
	...
	case spec.Join:
		...
		if hasMembershipChange {
			jr.Summary, err = snapshot.GetFrameSummary(ctx, delta.FrameID, device.UserID)
			if err != nil {
				logrus.WithError(err).Warn("failed to get frame summary")
			}
		}
		jr.Timeline.PrevBatch = &prevBatch
		jr.Timeline.Events = synctypes.ToClientEvents(xtools.ToPDUs(events), eventFormat, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
		...
		jr.State.Events = synctypes.ToClientEvents(xtools.ToPDUs(delta.StateEvents), eventFormat, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, frameID, senderID)
		})
		req.Response.Frames.Join[delta.FrameID] = jr
	...
	case spec.Peek:
		...
		req.Response.Frames.Peek[delta.FrameID] = jr
	...
	case spec.Ban:
		...
		lr.Timeline.Events = synctypes.ToClientEvents(xtools.ToPDUs(events), eventFormat, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, frameID, senderID)
		})
		// If we are limited by the filter AND the history visibility filter
		// didn't "remove" events, return that the response is limited.
		lr.Timeline.Limited = limited && len(events) == len(recentEvents)
		lr.State.Events = synctypes.ToClientEvents(xtools.ToPDUs(delta.StateEvents), eventFormat, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
			return p.rsAPI.QueryUserIDForSender(ctx, frameID, senderID)
		})
		req.Response.Frames.Leave[delta.FrameID] = lr
	}

	return latestPosition, nil
}

func (p *PDUStreamProvider) updatePowerLevelEvent(ctx context.Context, ev *rstypes.HeaderedEvent, eventFormat synctypes.ClientEventFormat) (xtools.PDU, error) {
	pls, err := xtools.NewPowerLevelContentFromEvent(ev)
	if err != nil {
		return nil, err
	}
	...
	for user, level := range pls.Users {
		validFrameID, _ := spec.NewFrameID(ev.FrameID())
		if eventFormat != synctypes.FormatSyncFederation {
			userID, err = p.rsAPI.QueryUserIDForSender(ctx, *validFrameID, spec.SenderID(user))
			if err != nil {
				return nil, err
			}
			user = userID.String()
	...
	if !prevContent.Exists() {
		var evNew xtools.PDU
		evNew, err = xtools.MustGetFrameVersion(ev.Version()).NewEventFromTrustedJSON(newEv, false)
		if err != nil {
			return nil, err
		}
	...
	var evNew xtools.PDU
	evNew, err = xtools.MustGetFrameVersion(ev.Version()).NewEventFromTrustedJSONWithEventID(ev.EventID(), newEv, false)
	if err != nil {
		return nil, err
	}
...
// applyHistoryVisibilityFilter gets the current frame state and supplies it to ApplyHistoryVisibilityFilter, to make
...
func applyHistoryVisibilityFilter(
	...
	if len(stateTypes) > 0 {
		...
		stateEvents, err := snapshot.CurrentState(ctx, frameID, &filter, nil)
		if err != nil {
			return nil, fmt.Errorf("failed to get current frame state for history visibility calculation: %w", err)
		}
...
func (p *PDUStreamProvider) getJoinResponseForCompleteSync(
	...
	stateEvents, err := snapshot.CurrentState(ctx, frameID, stateFilter, excludingEventIDs)
	if err != nil {
		return jr, err
	}

	jr.Summary, err = snapshot.GetFrameSummary(ctx, frameID, device.UserID)
	if err != nil {
		logrus.WithError(err).Warn("failed to get frame summary")
	}
	...
	// "Can sync a frame with a message with a transaction id" - which does a complete sync to check.
	recentEvents := snapshot.StreamEventsToEvents(ctx, device, recentStreamEvents, p.rsAPI)

	events := recentEvents
	// Only apply history visibility checks if the response is for joined frames
	if !isPeek {
		events, err = applyHistoryVisibilityFilter(ctx, snapshot, p.rsAPI, frameID, device.UserID, recentEvents)
		if err != nil {
			logrus.WithError(err).Error("unable to apply history visibility filter")
		}
	}
	...
	if len(events) > 0 {
		...
		event := events[0]
		// If this is the beginning of the frame, we can't go back further. We're going to return
	...
	jr.State.Events = synctypes.ToClientEvents(xtools.ToPDUs(stateEvents), eventFormat, func(frameID spec.FrameID, senderID spec.SenderID) (*spec.UserID, error) {
		return p.rsAPI.QueryUserIDForSender(ctx, frameID, senderID)
	})
	return jr, nil
}
...
func (p *PDUStreamProvider) lazyLoadMembers(
	ctx context.Context, snapshot storage.DatabaseTransaction, frameID string,
	...
	for _, event := range timelineEvents {
		...
		if _, ok := p.lazyLoadCache.IsLazyLoadedUserCached(device, frameID, string(event.SenderID())); !ok {
	...
	for _, event := range stateEvents {
		if event.Type() == spec.MFrameMember && event.StateKey() != nil {
			...
			userID := ""
			stateKeyUserID, queryErr := p.rsAPI.QueryUserIDForSender(ctx, *validFrameID, spec.SenderID(*event.StateKey()))
			...
			if _, ok := timelineUsers[userID]; ok || isGappedIncremental || userID == device.UserID {
				...
				if !stateFilter.IncludeRedundantMembers {
					p.lazyLoadCache.StoreLazyLoadedUser(device, frameID, userID, event.EventID())
				}
	...
	for _, membership := range memberships {
		p.lazyLoadCache.StoreLazyLoadedUser(device, frameID, *membership.StateKey(), membership.EventID())
	}
...
func (p *PDUStreamProvider) addIgnoredUsersToFilter(ctx context.Context, snapshot storage.DatabaseTransaction, req *types.SyncRequest, eventFilter *synctypes.FrameEventFilter) error {
...
func removeDuplicates[T xtools.PDU](stateEvents, recentEvents []T) []T {
