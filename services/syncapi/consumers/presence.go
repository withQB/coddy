...
func NewPresenceConsumer(
	...
	return &PresenceConsumer{
		...
		nats:          nats,
		jetstream:     js,
		durable:       cfg.Coddy.JetStream.Durable("SyncAPIPresenceConsumer"),
		presenceTopic: cfg.Coddy.JetStream.Prefixed(jetstream.OutputPresenceEvent),
		requestTopic:  cfg.Coddy.JetStream.Prefixed(jetstream.RequestPresence),
		db:            db,
		notifier:      notifier,
		stream:        stream,
		deviceAPI:     deviceAPI,
...
func (s *PresenceConsumer) Start() error {
	...
	if !s.cfg.Coddy.Presence.EnableInbound && !s.cfg.Coddy.Presence.EnableOutbound {
		return nil
	}
