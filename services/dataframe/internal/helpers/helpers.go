...
import (
	...
	"github.com/withqb/coddy/services/dataframe/storage/shared"
...
func UpdateToInviteMembership(
	...
	if needsSending {
		...
		// frame event stream. This ensures that the consumers only have to
...
func IsServerCurrentlyInFrame(ctx context.Context, db storage.Database, querier api.QuerySenderIDAPI, serverName spec.ServerName, frameID string) (bool, error) {
	...
	if info == nil {
		return false, fmt.Errorf("unknown frame %s", frameID)
	}

	if serverName == "" {
		return db.GetLocalServerInFrame(ctx, info.FrameNID)
	}

	eventNIDs, err := db.GetMembershipEventNIDsForFrame(ctx, info.FrameNID, true, false)
	if err != nil {
		return false, err
	}

	events, err := db.Events(ctx, info.FrameVersion, eventNIDs)
	if err != nil {
		return false, err
	}
	gmslEvents := make([]xtools.PDU, len(events))
	for i := range events {
...
func IsInvitePending(
	...
	frameID string, senderID spec.SenderID,
) (bool, spec.SenderID, string, xtools.PDU, error) {
	// Look up the frame NID for the supplied frame ID.
	info, err := db.FrameInfo(ctx, frameID)
	if err != nil {
		return false, "", "", nil, fmt.Errorf("r.DB.FrameInfo: %w", err)
	}
	if info == nil {
		return false, "", "", nil, fmt.Errorf("cannot get FrameInfo: unknown frame ID %s", frameID)
	}
	...
	// Let's see if we have an event active for the user in the frame. If
	// we do then it will contain a server name that we can direct the
	// send_leave to.
	senderUserNIDs, eventIDs, eventJSON, err := db.GetInvitesForUser(ctx, info.FrameNID, targetUserNID)
	if err != nil {
	...
	verImpl, err := xtools.GetFrameVersion(info.FrameVersion)
	if err != nil {
...
// only keep the "m.frame.member" events with a "join" membership. These events are returned.
// Returns an error if there was an issue fetching the events.
func GetMembershipsAtState(
	ctx context.Context, db storage.FrameDatabase, frameInfo *types.FrameInfo, stateEntries []types.StateEntry, joinedOnly bool,
) ([]types.Event, error) {

	var eventNIDs types.EventNIDs
	for _, entry := range stateEntries {
		// Filter the events to retrieve to only keep the membership events
		if entry.EventTypeNID == types.MFrameMemberNID {
...
func StateBeforeEvent(ctx context.Context, db storage.Database, info *types.FrameInfo, eventNID types.EventNID, querier api.QuerySenderIDAPI) ([]types.StateEntry, error) {
	frameState := state.NewStateResolution(db, info, querier)
	...
	return frameState.LoadCombinedStateAfterEvents(ctx, prevState)
}

func MembershipAtEvent(ctx context.Context, db storage.FrameDatabase, info *types.FrameInfo, eventIDs []string, stateKeyNID types.EventStateKeyNID, querier api.QuerySenderIDAPI) (map[string][]types.StateEntry, error) {
	frameState := state.NewStateResolution(db, info, querier)
	// Fetch the state as it was when this event was fired
	return frameState.LoadMembershipAtEvent(ctx, eventIDs, stateKeyNID)
}

func LoadEvents(
	ctx context.Context, db storage.FrameDatabase, frameInfo *types.FrameInfo, eventNIDs []types.EventNID,
) ([]xtools.PDU, error) {
	if frameInfo == nil {
		return nil, types.ErrorInvalidFrameInfo
	}
	...
	result := make([]xtools.PDU, len(stateEvents))
...
func LoadStateEvents(
	ctx context.Context, db storage.FrameDatabase, frameInfo *types.FrameInfo, stateEntries []types.StateEntry,
) ([]xtools.PDU, error) {
	eventNIDs := make([]types.EventNID, len(stateEntries))
	for i := range stateEntries {
		eventNIDs[i] = stateEntries[i].EventNID
	}
	return LoadEvents(ctx, db, frameInfo, eventNIDs)
}

func CheckServerAllowedToSeeEvent(
	ctx context.Context, db storage.Database, info *types.FrameInfo, frameID string, eventID string, serverName spec.ServerName, isServerInFrame bool, querier api.QuerySenderIDAPI,
) (bool, error) {
	...
	case tables.OptimisationNotSupportedError:
		...
		stateAtEvent, err = slowGetHistoryVisibilityState(ctx, db, info, frameID, eventID, serverName, querier)
		if err != nil {
			return false, err
		}
	default:
		...
		case types.MissingStateError:
			...
			// https://github.com/withqb/synapse/blob/aec87a0f9369a3015b2a53469f88d1de274e8b71/synapse/visibility.py#L654-L655
			return true, nil
		default:
	...
	return auth.IsServerAllowed(ctx, querier, serverName, isServerInFrame, stateAtEvent), nil
}

func slowGetHistoryVisibilityState(
	ctx context.Context, db storage.Database, info *types.FrameInfo, frameID, eventID string, serverName spec.ServerName, querier api.QuerySenderIDAPI,
) ([]xtools.PDU, error) {
	frameState := state.NewStateResolution(db, info, querier)
	stateEntries, err := frameState.LoadStateAtEvent(ctx, eventID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, nil
		}
		return nil, fmt.Errorf("frameState.LoadStateAtEvent: %w", err)
	}

	// Extract all of the event state key NIDs from the frame state.
	...
	for nid, key := range stateKeys {
		if key != "" {
			userID, err := querier.QueryUserIDForSender(ctx, *validFrameID, spec.SenderID(key))
	...
	// Now filter through all of the state events for the frame.
...
func ScanEventTree(
	...
	var checkedServerInFrame bool
	var isServerInFrame bool
...
BFSLoop:
	for len(front) > 0 {
		...
		if !checkedServerInFrame && len(events) > 0 {
			// It's nasty that we have to extract the frame ID from an event, but many federation requests
			// only talk in event IDs, no frame IDs at all (!!!)
			ev := events[0]
		...
		for _, ev := range events {
			...
			for _, pre = range ev.PrevEventIDs() {
				...
				if !visited[pre] {
					visited[pre] = true
					allowed, err = CheckServerAllowedToSeeEvent(ctx, db, info, ev.FrameID(), pre, serverName, isServerInFrame, querier)
					if err != nil {
						xutil.GetLogger(ctx).WithField("server", serverName).WithField("event_id", pre).WithError(err).Error(
					...
					if !allowed {
						xutil.GetLogger(ctx).WithField("server", serverName).WithField("event_id", pre).Info("Not allowed to see event")
...
func QueryLatestEventsAndState(
	...
	frameInfo, err := db.FrameInfo(ctx, request.FrameID)
	if err != nil {
		return err
	}
	...
	response.FrameVersion = frameInfo.FrameVersion

	var currentStateSnapshotNID types.StateSnapshotNID
	response.LatestEvents, currentStateSnapshotNID, response.Depth, err =
		db.LatestEventIDs(ctx, frameInfo.FrameNID)
	if err != nil {
		return err
	}
	...
	stateEvents, err := LoadStateEvents(ctx, db, frameInfo, stateEntries)
	if err != nil {
		return err
	}
