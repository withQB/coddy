...
// user affected by a change in the current state of the frame.
...
func (r *Inputer) updateMemberships(
	...
	// TDO: Maybe add a membership key to the events table so we can load that
...
func (r *Inputer) updateMembership(
	...
	// it (a thing that ordinarily shouldn't happen in Coddy). However, state
	...
	// the frame even though the frame state said otherwise, and this would prevent
	// the user from being able to attempt to rejoin the frame without modifying
	...
	case spec.Invite:
		return helpers.UpdateToInviteMembership(mu, add, updates, updater.FrameVersion())
	case spec.Join:
...
func (r *Inputer) isLocalTarget(ctx context.Context, event *types.Event) bool {
	...
	if statekey := event.StateKey(); statekey != nil {
		...
		userID, err := r.Queryer.QueryUserIDForSender(ctx, *validFrameID, spec.SenderID(*statekey))
...
func updateToJoinMembership(
	...
	// by studying the state changes in the frame event stream.
...
func membershipChanges(removed, added []types.StateEntry) []stateChange {
	...
	for _, c := range changes {
		if c.EventTypeNID == types.MFrameMemberNID {
