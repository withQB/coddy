...
func (r *Inputer) startWorkerForFrame(frameID string) {
	...
	if !loaded || w.subscription == nil {
		consumer := r.Cfg.Coddy.JetStream.Prefixed("FrameInput" + jetstream.Tokenise(w.frameID))
		subject := r.Cfg.Coddy.JetStream.Prefixed(jetstream.InputFrameEventSubj(w.frameID))
		...
		if _, err := w.r.JetStream.AddConsumer(
			r.Cfg.Coddy.JetStream.Prefixed(jetstream.InputFrameEvent),
			&nats.ConsumerConfig{
				Durable:           consumer,
...
func (r *Inputer) Start() error {
	...
	stream := r.Cfg.Coddy.JetStream.Prefixed(jetstream.InputFrameEvent)
...
func (r *Inputer) queueInputFrameEvents(
	...
	for _, e := range request.InputFrameEvents {
		...
		subj := r.Cfg.Coddy.JetStream.Prefixed(jetstream.InputFrameEventSubj(frameID))
		msg := &nats.Msg{
			Subject: subj,
			Header:  nats.Header{},
		}
