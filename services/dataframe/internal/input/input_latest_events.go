...
// updateLatestEvents updates the list of latest events for this frame in the database and writes the
// event to the output log.
// The latest events are the events that aren't referenced by another event in the database:
//
//	Time goes down the page. 1 is the m.frame.create event (root).
...
// frame. It mostly just ferries state between the various function calls.
...
type latestEventsUpdater struct {
	...
	// The latest events in the frame after processing this event.
	...
	// frame as a result of processing this event. They are sorted lists.
...
func (u *latestEventsUpdater) doUpdateLatestEvents() error {
	...
	// state snapshot from somewhere else, e.g. a federated frame join,
	...
	if err = u.updater.SetLatestEvents(u.frameInfo.FrameNID, u.latest, u.stateAtEvent.EventNID, u.newStateNID); err != nil {
		return fmt.Errorf("u.updater.SetLatestEvents: %w", err)
	}

	update, err := u.makeOutputNewFrameEvent()
	if err != nil {
		return fmt.Errorf("u.makeOutputNewFrameEvent: %w", err)
	}
	...
	if err = u.api.OutputProducer.ProduceFrameEvents(u.event.FrameID(), updates); err != nil {
...
func (u *latestEventsUpdater) latestState() error {
	...
	var err error
	frameState := state.NewStateResolution(u.updater, u.frameInfo, u.api.Queryer)
	...
	// using the correct state resolution algorithm for the frame.
	u.newStateNID, err = frameState.CalculateAndStoreStateAfterEvents(
		ctx, latestStateAtEvents,
	)
	if err != nil {
		return fmt.Errorf("frameState.CalculateAndStoreStateAfterEvents: %w", err)
	}
	...
	// frame state first. If it doesn't rewrite the state then we will work out
	...
	if u.rewritesState {
		...
		u.added, err = frameState.LoadStateAtSnapshot(ctx, u.newStateNID)
		if err != nil {
			return fmt.Errorf("frameState.LoadStateAtSnapshot: %w", err)
		}
	} else {
		u.removed, u.added, err = frameState.DifferenceBetweeenStateSnapshots(
			ctx, u.oldStateNID, u.newStateNID,
		)
		if err != nil {
			return fmt.Errorf("frameState.DifferenceBetweenStateSnapshots: %w", err)
		}
	}
	...
	// adds.
	u.stateBeforeEventRemoves, u.stateBeforeEventAdds, err = frameState.DifferenceBetweeenStateSnapshots(
		ctx, u.newStateNID, u.stateAtEvent.BeforeStateSnapshotNID,
	)
	if err != nil {
		return fmt.Errorf("frameState.DifferenceBetweeenStateSnapshots: %w", err)
	}
...
func (u *latestEventsUpdater) makeOutputNewFrameEvent() (*api.OutputEvent, error) {
	latestEventIDs := make([]string, len(u.latest))
	for i := range u.latest {
		latestEventIDs[i] = u.latest[i].EventID
	}

	ore := api.OutputNewFrameEvent{
	...
	return &api.OutputEvent{
		Type:         api.OutputTypeNewFrameEvent,
		NewFrameEvent: &ore,
	}, nil
}
...
func (u *latestEventsUpdater) stateEventMap() (map[types.EventNID]string, error) {
	...
	stateEventNIDs = stateEventNIDs[:xutil.SortAndUnique(stateEventNIDs)]
