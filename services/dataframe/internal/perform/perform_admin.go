...
func (r *Admin) PerformAdminEvacuateFrame(
	...
	for _, memberEvent := range memberEvents {
		...
		identity, err = r.Cfg.Coddy.SigningIdentityFor(senderDomain)
		if err != nil {
			continue
		}
...
func (r *Admin) PerformAdminEvacuateUser(
	...
	if !r.Cfg.Coddy.IsLocalServerName(fullUserID.Domain()) {
...
func (r *Admin) PerformAdminDownloadState(
	...
	inputReq.InputFrameEvents = append(inputReq.InputFrameEvents, api.InputFrameEvent{
		Kind:          api.KindNew,
		Event:         ev,
		Origin:        r.Cfg.Coddy.ServerName,
		HasState:      true,
		StateEventIDs: stateIDs,
		SendAsServer:  string(r.Cfg.Coddy.ServerName),
	})
