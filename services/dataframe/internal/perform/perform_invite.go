...
func (r *Inviter) ProcessInviteMembership(
	...
	isTargetLocal := r.Cfg.Coddy.IsLocalServerName(userID.Domain())
...
func (r *Inviter) PerformInvite(
	...
	if !r.Cfg.Coddy.IsLocalServerName(req.InviteInput.Inviter.Domain()) {
		return api.ErrInvalidID{Err: fmt.Errorf("the invite must be from a local user")}
	}

	isTargetLocal := r.Cfg.Coddy.IsLocalServerName(req.InviteInput.Invitee.Domain())
