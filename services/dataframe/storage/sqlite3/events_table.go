...
const eventsSchema = `
  CREATE TABLE IF NOT EXISTS dataframe_events (
...
const insertEventSQL = `
	INSERT INTO dataframe_events (frame_nid, event_type_nid, event_state_key_nid, event_id, auth_event_nids, depth, is_rejected)
...
const selectEventSQL = "" +
	"SELECT event_nid, state_snapshot_nid FROM dataframe_events WHERE event_id = $1"

const bulkSelectSnapshotsForEventIDsSQL = "" +
	"SELECT event_id, state_snapshot_nid FROM dataframe_events WHERE event_id IN ($1)"

// Bulk lookup of events by string ID.
// Sort by the numeric IDs for event type and state key.
// This means we can use binary search to lookup entries by type and state key.
const bulkSelectStateEventByIDSQL = "" +
	"SELECT event_type_nid, event_state_key_nid, event_nid FROM dataframe_events" +
	" WHERE event_id IN ($1)" +
...
const bulkSelectStateAtEventByIDSQL = "" +
	"SELECT event_type_nid, event_state_key_nid, event_nid, state_snapshot_nid, is_rejected FROM dataframe_events" +
	" WHERE event_id IN ($1)"

const updateEventStateSQL = "" +
	"UPDATE dataframe_events SET state_snapshot_nid = $1 WHERE event_nid = $2"

const selectEventSentToOutputSQL = "" +
	"SELECT sent_to_output FROM dataframe_events WHERE event_nid = $1"

const updateEventSentToOutputSQL = "" +
	"UPDATE dataframe_events SET sent_to_output = TRUE WHERE event_nid = $1"

const selectEventIDSQL = "" +
	"SELECT event_id FROM dataframe_events WHERE event_nid = $1"
...
const bulkSelectStateAtEventAndReferenceSQL = "" +
	...
	" FROM dataframe_events WHERE event_nid IN ($1)"

const bulkSelectEventIDSQL = "" +
	"SELECT event_nid, event_id FROM dataframe_events WHERE event_nid IN ($1)"

const bulkSelectEventNIDSQL = "" +
	"SELECT event_id, event_nid, frame_nid FROM dataframe_events WHERE event_id IN ($1)"

const bulkSelectUnsentEventNIDSQL = "" +
	"SELECT event_id, event_nid, frame_nid FROM dataframe_events WHERE sent_to_output = 0 AND event_id IN ($1)"

const selectMaxEventDepthSQL = "" +
	"SELECT COALESCE(MAX(depth) + 1, 0) FROM dataframe_events WHERE event_nid IN ($1)"

const selectFrameNIDsForEventNIDsSQL = "" +
	"SELECT event_nid, frame_nid FROM dataframe_events WHERE event_nid IN ($1)"

const selectEventRejectedSQL = "" +
	"SELECT is_rejected FROM dataframe_events WHERE frame_nid = $1 AND event_id = $2"
...
type eventStatements struct {
	...
	//selectFrameNIDsForEventNIDsStmt       *sql.Stmt
}
...
func CreateEventsTable(db *sql.DB) error {
	...
	var cName string
	migrationName := "dataframe: drop column reference_sha from dataframe_events"
	err = db.QueryRowContext(context.Background(), `SELECT p.name FROM sqlite_master AS m JOIN pragma_table_info(m.name) AS p WHERE m.name = 'dataframe_events' AND p.name = 'reference_sha256'`).Scan(&cName)
	if err != nil {
...
func PrepareEventsTable(db *sql.DB) (tables.Events, error) {
	...
	return s, sqlutil.StatementList{
		...
		//{&s.selectFrameNIDForEventNIDStmt, selectFrameNIDForEventNIDSQL},
...
func (s *eventStatements) InsertEvent(
	...
	err := insertStmt.QueryRowContext(
		ctx, int64(frameNID), int64(eventTypeNID), int64(eventStateKeyNID),
...
func (s *eventStatements) BulkSelectStateAtEventByID(
	...
	for ; rows.Next(); i++ {
		...
		isCreate := result.EventTypeNID == types.MFrameCreateNID && result.EventStateKeyNID == 1
...
// BulkSelectEventNIDs returns a map from string event ID to numeric event ID
// only for events that haven't already been sent to the dataframe output.
// If an event ID is not in the database then it is omitted from the map.
...
func (s *eventStatements) bulkSelectEventNID(ctx context.Context, txn *sql.Tx, eventIDs []string, onlyUnsent bool) (map[string]types.EventMetadata, error) {
	...
	var frameNID int64
	for rows.Next() {
		if err = rows.Scan(&eventID, &eventNID, &frameNID); err != nil {
			return nil, err
		}
		results[eventID] = types.EventMetadata{
			EventNID: types.EventNID(eventNID),
			FrameNID:  types.FrameNID(frameNID),
		}
	}
	return results, nil
}
...
func (s *eventStatements) SelectFrameNIDsForEventNIDs(
	ctx context.Context, txn *sql.Tx, eventNIDs []types.EventNID,
) (map[types.EventNID]types.FrameNID, error) {
	sqlStr := strings.Replace(selectFrameNIDsForEventNIDsSQL, "($1)", sqlutil.QueryVariadic(len(eventNIDs)), 1)
	sqlPrep, err := s.db.Prepare(sqlStr)
	if err != nil {
		return nil, err
	}
	...
	defer internal.CloseAndLogIfError(ctx, rows, "selectFrameNIDsForEventNIDsStmt: rows.close() failed")
	result := make(map[types.EventNID]types.FrameNID)
	var eventNID types.EventNID
	var frameNID types.FrameNID
	for rows.Next() {
		if err = rows.Scan(&eventNID, &frameNID); err != nil {
			return nil, err
		}
		result[eventNID] = frameNID
	}
	return result, nil
}
...
func (s *eventStatements) SelectEventRejected(
	ctx context.Context, txn *sql.Tx, frameNID types.FrameNID, eventID string,
) (rejected bool, err error) {
	stmt := sqlutil.TxStmt(txn, s.selectEventRejectedStmt)
	err = stmt.QueryRowContext(ctx, frameNID, eventID).Scan(&rejected)
	return
}
