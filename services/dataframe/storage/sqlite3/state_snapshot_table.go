...
const stateSnapshotSchema = `
  CREATE TABLE IF NOT EXISTS dataframe_state_snapshots (
	...
	-- The frame NID that the snapshot belongs to.
...
const insertStateSQL = `
	INSERT INTO dataframe_state_snapshots (state_snapshot_hash, frame_nid, state_block_nids)
	  VALUES ($1, $2, $3)
	  ON CONFLICT (state_snapshot_hash) DO UPDATE SET frame_nid=$2
...
const bulkSelectStateBlockNIDsSQL = "" +
	"SELECT state_snapshot_nid, state_block_nids FROM dataframe_state_snapshots" +
	" WHERE state_snapshot_nid IN ($1) ORDER BY state_snapshot_nid ASC"

const selectStateBlockNIDsForFrameNID = "" +
	"SELECT state_block_nids FROM dataframe_state_snapshots WHERE frame_nid = $1"
...
func PrepareStateSnapshotTable(db *sql.DB) (*stateSnapshotStatements, error) {
	...
	return s, sqlutil.StatementList{
		...
		{&s.selectStateBlockNIDsStmt, selectStateBlockNIDsForFrameNID},
	}.Prepare(db)
}

func (s *stateSnapshotStatements) InsertState(
	ctx context.Context, txn *sql.Tx, frameNID types.FrameNID, stateBlockNIDs types.StateBlockNIDs,
) (stateNID types.StateSnapshotNID, err error) {
	if stateBlockNIDs == nil {
		stateBlockNIDs = []types.StateBlockNID{} // zero slice to not store 'null' in the DB
	}
	stateBlockNIDs = stateBlockNIDs[:xutil.SortAndUnique(stateBlockNIDs)]
	stateBlockNIDsJSON, err := json.Marshal(stateBlockNIDs)
	if err != nil {
		return
	}
	insertStmt := sqlutil.TxStmt(txn, s.insertStateStmt)
	err = insertStmt.QueryRowContext(ctx, stateBlockNIDs.Hash(), int64(frameNID), string(stateBlockNIDsJSON)).Scan(&stateNID)
	if err != nil {
		return 0, err
	}
	return
...
func (s *stateSnapshotStatements) BulkSelectMembershipForHistoryVisibility(ctx context.Context, txn *sql.Tx, userNID types.EventStateKeyNID, frameInfo *types.FrameInfo, eventIDs ...string) (map[string]*types.HeaderedEvent, error) {
	return nil, tables.OptimisationNotSupportedError
}

func (s *stateSnapshotStatements) selectStateBlockNIDsForFrameNID(
	ctx context.Context, txn *sql.Tx, frameNID types.FrameNID,
) ([]types.StateBlockNID, error) {
	var res []types.StateBlockNID
	rows, err := sqlutil.TxStmt(txn, s.selectStateBlockNIDsStmt).QueryContext(ctx, frameNID)
	if err != nil {
		return res, nil
	}
	defer internal.CloseAndLogIfError(ctx, rows, "selectStateBlockNIDsForFrameNID: rows.close() failed")
