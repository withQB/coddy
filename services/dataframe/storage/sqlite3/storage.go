...
// A Database is used to store frame events and stream offsets.
...
func executeMigration(ctx context.Context, db *sql.DB) error {
	...
	var cName string
	err := db.QueryRowContext(ctx, `SELECT p.name FROM sqlite_master AS m JOIN pragma_table_info(m.name) AS p WHERE m.name = 'dataframe_state_block' AND p.name = 'event_nid'`).Scan(&cName)
	if err != nil {
...
func (d *Database) create(db *sql.DB) error {
	...
	if err := CreateFramesTable(db); err != nil {
		return err
	}
	...
	if err := CreateFrameAliasesTable(db); err != nil {
		return err
	}
	...
	if err := CreateUserFrameKeysTable(db); err != nil {
		return err
	}
...
func (d *Database) prepare(db *sql.DB, writer sqlutil.Writer, cache caching.DataFrameCaches) error {
	...
	frameAliases, err := PrepareFrameAliasesTable(db)
	if err != nil {
		return err
	}
	...
	userFrameKeys, err := PrepareUserFrameKeysTable(db)
	if err != nil {
		return err
	}
	...
	d.Database = shared.Database{
		...
		FrameAliasesTable:   frameAliases,
		InvitesTable:       invites,
		MembershipTable:    membership,
		PublishedTable:     published,
		GetFrameUpdaterFn:   d.GetFrameUpdater,
		Purge:              purge,
		UserFrameKeyTable:   userFrameKeys,
	}
	return nil
}
...
func (d *Database) GetFrameUpdater(
	...
	return shared.NewFrameUpdater(ctx, &d.Database, nil, frameInfo)
}
...
func (d *Database) MembershipUpdater(
	...
	// same DB anyway, and we only execute updates sequentially, the only worries
	// are for rolling back when things go wrong. (atomicity)
	return shared.NewMembershipUpdater(ctx, &d.Database, nil, frameID, targetUserID, targetLocal, frameVersion)
}
