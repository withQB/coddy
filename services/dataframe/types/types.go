// Package types provides the types that are used internally within the dataframe.
package types
...
// FrameNID is a numeric ID for a frame.
type FrameNID int64
...
func (a StateKeyTuple) IsCreate() bool {
	return a.EventTypeNID == MFrameCreateNID && a.EventStateKeyNID == EmptyStateKeyNID
}
...
func (s StateKeyTupleSorter) TypesAndStateKeysAsArrays() (eventTypeNIDs []int64, eventStateKeyNIDs []int64) {
	...
	eventTypeNIDs = eventTypeNIDs[:xutil.SortAndUnique(int64Sorter(eventTypeNIDs))]
	eventStateKeyNIDs = eventStateKeyNIDs[:xutil.SortAndUnique(int64Sorter(eventStateKeyNIDs))]
	return
}
...
// A StateEntry is an entry in the frame state of a coddy frame.
...
// StateAtEvent is the state before and after a coddy event.
...
// StateAtEventAndReference is StateAtEvent and xtools.EventReference glued together.
// It is used when looking up the latest events in a frame in the database.
// The xtools.EventReference is used to check whether a new event references the event.
// The StateAtEvent is used to construct the current state of the frame from the latest events.
...
// An Event is a xtools.Event with the numeric event ID attached.
...
const (
	// MFrameCreateNID is the numeric ID for the "m.frame.create" event type.
	MFrameCreateNID = 1
	// MFramePowerLevelsNID is the numeric ID for the "m.frame.power_levels" event type.
	MFramePowerLevelsNID = 2
	// MFrameJoinRulesNID is the numeric ID for the "m.frame.join_rules" event type.
	MFrameJoinRulesNID = 3
	// MFrameThirdPartyInviteNID is the numeric ID for the "m.frame.third_party_invite" event type.
	MFrameThirdPartyInviteNID = 4
	// MFrameMemberNID is the numeric ID for the "m.frame.member" event type.
	MFrameMemberNID = 5
	// MFrameRedactionNID is the numeric ID for the "m.frame.redaction" event type.
	MFrameRedactionNID = 6
	// MFrameHistoryVisibilityNID is the numeric ID for the "m.frame.history_visibility" event type.
	MFrameHistoryVisibilityNID = 7
)
...
// A MissingEventError is an error that happened because the dataframe was
...
// A MissingStateError is an error that happened because the dataframe was
...
func (r *FrameInfo) StateSnapshotNID() StateSnapshotNID {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.stateSnapshotNID
}

func (r *FrameInfo) IsStub() bool {
	r.mu.RLock()
	defer r.mu.RUnlock()
	return r.isStub
}

func (r *FrameInfo) SetStateSnapshotNID(nid StateSnapshotNID) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.stateSnapshotNID = nid
}

func (r *FrameInfo) SetIsStub(isStub bool) {
	r.mu.Lock()
	defer r.mu.Unlock()
	r.isStub = isStub
}

func (r *FrameInfo) CopyFrom(r2 *FrameInfo) {
	r.mu.Lock()
	defer r.mu.Unlock()
	...
	r.FrameNID = r2.FrameNID
	r.FrameVersion = r2.FrameVersion
	r.stateSnapshotNID = r2.stateSnapshotNID
	r.isStub = r2.isStub
}

var ErrorInvalidFrameInfo = fmt.Errorf("frame info is invalid")
