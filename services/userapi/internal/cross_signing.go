...
import (
	...
	"github.com/withqb/coddy/services/userapi/types"
...
func (a *UserInternalAPI) PerformUploadDeviceSignatures(ctx context.Context, req *api.PerformUploadDeviceSignaturesRequest, res *api.PerformUploadDeviceSignaturesResponse) {
	...
	selfSignatures := map[string]map[xtools.KeyID]fclient.CrossSigningForKeyOrDevice{}
	otherSignatures := map[string]map[xtools.KeyID]fclient.CrossSigningForKeyOrDevice{}
			...
			case *fclient.CrossSigningKey:
				if key.UserID == req.UserID {
					if _, ok := selfSignatures[userID]; !ok {
						selfSignatures[userID] = map[xtools.KeyID]fclient.CrossSigningForKeyOrDevice{}
					}
					selfSignatures[userID][keyID] = keyOrDevice
				} else {
					if _, ok := otherSignatures[userID]; !ok {
						otherSignatures[userID] = map[xtools.KeyID]fclient.CrossSigningForKeyOrDevice{}
					}
...
func (a *UserInternalAPI) processSelfSignatures(
	ctx context.Context,
	signatures map[string]map[xtools.KeyID]fclient.CrossSigningForKeyOrDevice,
) error {
		...
		for targetKeyID, signature := range forTargetUserID {
			...
			case *fclient.CrossSigningKey:
				for keyID := range sig.Keys {
					...
					if len(split) > 1 && xtools.KeyID(split[1]) == targetKeyID {
	...
	for targetUserID, forTargetUserID := range signatures {
		for _, signature := range forTargetUserID {
			...
			default:
				...
				// xtools.DeviceKeys, which doesn't make sense.
			}
		}
	}
...
func (a *UserInternalAPI) crossSigningKeysFromDatabase(
	...
	for targetUserID := range req.UserToDevices {
		...
		for keyType, key := range keys {
			...
			appendSignature := func(originUserID string, originKeyID xtools.KeyID, signature spec.Base64Bytes) {
				if key.Signatures == nil {
					key.Signatures = types.CrossSigningSigMap{}
				}
				if _, ok := key.Signatures[originUserID]; !ok {
					key.Signatures[originUserID] = make(map[xtools.KeyID]spec.Base64Bytes)
				}
			...
			for sourceUserID, forSourceUser := range sigMap {
				for sourceKeyID, sourceSig := range forSourceUser {
					if res.Signatures == nil {
						res.Signatures = map[string]map[xtools.KeyID]types.CrossSigningSigMap{}
					}
					if _, ok := res.Signatures[targetUserID]; !ok {
						res.Signatures[targetUserID] = map[xtools.KeyID]types.CrossSigningSigMap{}
					}
					...
					if _, ok := res.Signatures[targetUserID][targetKeyID][sourceUserID]; !ok {
						res.Signatures[targetUserID][targetKeyID][sourceUserID] = map[xtools.KeyID]spec.Base64Bytes{}
					}
