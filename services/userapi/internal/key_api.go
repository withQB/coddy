...
func (a *UserInternalAPI) PerformClaimKeys(ctx context.Context, req *api.PerformClaimKeysRequest, res *api.PerformClaimKeysResponse) {
	...
	for domain, local := range domainToDeviceKeys {
		if !a.Config.Coddy.IsLocalServerName(spec.ServerName(domain)) {
			continue
		}
		// claim local keys
		...
		xutil.GetLogger(ctx).WithField("keys_claimed", len(keys)).WithField("num_users", len(local)).Info("Claimed local keys")
...
func (a *UserInternalAPI) claimRemoteKeys(
	...
	xutil.GetLogger(ctx).Infof("Claiming remote keys from %d server(s)", len(domainToDeviceKeys))
	...
	for d, k := range domainToDeviceKeys {
		go func(domain string, keysToClaim map[string]map[string]string) {
			...
			claimKeyRes, err := a.FedClient.ClaimKeys(fedCtx, a.Config.Coddy.ServerName, spec.ServerName(domain), keysToClaim)

			mu.Lock()
			defer mu.Unlock()

			if err != nil {
				xutil.GetLogger(ctx).WithError(err).WithField("server", domain).Error("ClaimKeys failed")
...
func (a *UserInternalAPI) QueryKeys(ctx context.Context, req *api.QueryKeysRequest, res *api.QueryKeysResponse) {
	...
	for targetUserID, masterKey := range res.MasterKeys {
		if masterKey.Signatures == nil {
			masterKey.Signatures = map[string]map[xtools.KeyID]spec.Base64Bytes{}
		}
			...
			for sourceUserID, forSourceUser := range sigMap {
				for sourceKeyID, sourceSig := range forSourceUser {
					if _, ok := masterKey.Signatures[sourceUserID]; !ok {
						masterKey.Signatures[sourceUserID] = map[xtools.KeyID]spec.Base64Bytes{}
					}
	...
	for targetUserID, forUserID := range res.DeviceKeys {
		for targetKeyID, key := range forUserID {
			sigMap, err := a.KeyDatabase.CrossSigningSigsForTarget(ctx, req.UserID, targetUserID, xtools.KeyID(targetKeyID))
			if err != nil {
				// Stop executing the function if the context was canceled/the deadline was exceeded,
				// as we can't continue without a valid context.
				if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			...
			if deviceKey.Signatures == nil {
				deviceKey.Signatures = map[string]map[xtools.KeyID]spec.Base64Bytes{}
			}
			for sourceUserID, forSourceUser := range sigMap {
				for sourceKeyID, sourceSig := range forSourceUser {
					if _, ok := deviceKey.Signatures[sourceUserID]; !ok {
						deviceKey.Signatures[sourceUserID] = map[xtools.KeyID]spec.Base64Bytes{}
					}
	...
	for domain, userToDeviceMap := range domainToDeviceKeys {
		for userID, deviceIDs := range userToDeviceMap {
			...
			if len(deviceIDs) > 0 {
				...
				xutil.GetLogger(ctx).WithError(err).Error("populateResponseWithDeviceKeysFromDatabase")
			}
...
func (a *UserInternalAPI) queryRemoteKeysOnServer(
	...
	queryKeysResp, err := a.FedClient.QueryKeys(fedCtx, a.Config.Coddy.ServerName, spec.ServerName(serverName), devKeys)
	if err == nil {
...
func (a *UserInternalAPI) uploadLocalDeviceKeys(ctx context.Context, req *api.PerformUploadKeysRequest, res *api.PerformUploadKeysResponse) {
	...
	var toClean []xtools.KeyID
	for _, k := range existingKeys {
		if _, ok := existingDeviceMap[k.DeviceID]; !ok {
			toClean = append(toClean, xtools.KeyID(k.DeviceID))
		}
	}
	...
	if req.OnlyDisplayNameUpdates {
		...
		for _, key := range req.DeviceKeys {
			...
			_, serverName, err = xtools.SplitID('@', key.UserID)
			if err != nil {
				continue // ignore invalid users
			}
			if !a.Config.Coddy.IsLocalServerName(serverName) {
