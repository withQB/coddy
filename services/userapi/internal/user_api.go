...
import (
	...
	fedsenderapi "github.com/withqb/coddy/services/federationapi/api"
	...
	"github.com/withqb/coddy/services/userapi/producers"
	"github.com/withqb/coddy/services/userapi/storage"
	"github.com/withqb/coddy/services/userapi/storage/tables"
	userapiUtil "github.com/withqb/coddy/services/userapi/util"
...
func (a *UserInternalAPI) InputAccountData(ctx context.Context, req *api.InputAccountDataRequest, res *api.InputAccountDataResponse) error {
	local, domain, err := xtools.SplitID('@', req.UserID)
	if err != nil {
		return err
	}
	if !a.Config.Coddy.IsLocalServerName(domain) {
	...
	if err := a.SyncProducer.SendAccountData(req.UserID, eventutil.AccountData{
		...
		xutil.GetLogger(ctx).WithError(err).Error("a.SyncProducer.SendAccountData failed")
...
func (a *UserInternalAPI) setFullyRead(ctx context.Context, req *api.InputAccountDataRequest) error {
	...
	localpart, domain, err := xtools.SplitID('@', req.UserID)
	if err != nil {
...
func postRegisterJoinFrames(cfg *config.UserAPI, acc *api.Account, rsAPI rsapi.UserDataframeAPI) {
	...
	if acc.AccountType != api.AccountTypeAppService && acc.AppServiceID == "" {
		for frame := range cfg.AutoJoinFrames {
			userID := userutil.MakeUserID(acc.Localpart, cfg.Coddy.ServerName)
...
func (a *UserInternalAPI) PerformAccountCreation(ctx context.Context, req *api.PerformAccountCreationRequest, res *api.PerformAccountCreationResponse) error {
	...
	if serverName == "" {
		serverName = a.Config.Coddy.ServerName
	}
	if !a.Config.Coddy.IsLocalServerName(serverName) {
...
func (a *UserInternalAPI) PerformPasswordUpdate(ctx context.Context, req *api.PerformPasswordUpdateRequest, res *api.PerformPasswordUpdateResponse) error {
	if !a.Config.Coddy.IsLocalServerName(req.ServerName) {
...
func (a *UserInternalAPI) PerformDeviceDeletion(ctx context.Context, req *api.PerformDeviceDeletionRequest, res *api.PerformDeviceDeletionResponse) error {
	xutil.GetLogger(ctx).WithField("user_id", req.UserID).WithField("devices", req.DeviceIDs).Info("PerformDeviceDeletion")
	local, domain, err := xtools.SplitID('@', req.UserID)
	if err != nil {
		return err
	}
	...
	for _, keyID := range req.DeviceIDs {
		deleteReq.KeyIDs = append(deleteReq.KeyIDs, xtools.KeyID(keyID))
	}
...
func (a *UserInternalAPI) PerformLastSeenUpdate(
	...
	localpart, domain, err := xtools.SplitID('@', req.UserID)
	if err != nil {
		return fmt.Errorf("xtools.SplitID: %w", err)
	}
	if !a.Config.Coddy.IsLocalServerName(domain) {
...
func (a *UserInternalAPI) PerformDeviceUpdate(ctx context.Context, req *api.PerformDeviceUpdateRequest, res *api.PerformDeviceUpdateResponse) error {
	localpart, domain, err := xtools.SplitID('@', req.RequestingUserID)
	if err != nil {
	...
	if err == sql.ErrNoRows {
		...
		xutil.GetLogger(ctx).WithError(err).Error("deviceDB.GetDeviceByID failed")
		return err
	}
	...
	if err != nil {
		xutil.GetLogger(ctx).WithError(err).Error("deviceDB.UpdateDevice failed")
		return err
	}
...
func (a *UserInternalAPI) QueryProfile(ctx context.Context, userID string) (*authtypes.Profile, error) {
	local, domain, err := xtools.SplitID('@', userID)
	if err != nil {
		return nil, err
	}
	if !a.Config.Coddy.IsLocalServerName(domain) {
...
func (a *UserInternalAPI) queryAppServiceToken(ctx context.Context, token, appServiceUserID string) (*api.Device, error) {
	...
	localpart, domain, err := userutil.ParseUsernameParam(appServiceUserID, a.Config.Coddy)
	if err != nil {
		return nil, err
	}
