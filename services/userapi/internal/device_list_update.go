...
func (u *DeviceListUpdater) Update(ctx context.Context, event xtools.DeviceListUpdateEvent) error {
...
func (u *DeviceListUpdater) update(ctx context.Context, event xtools.DeviceListUpdateEvent) (bool, error) {
	...
	if exists || event.Deleted {
		...
		if err = u.db.DeviceKeysJSON(ctx, existingKeys); err != nil {
			...
			xutil.GetLogger(ctx).WithError(err).WithField("user_id", event.UserID).Errorf(
...
func (u *DeviceListUpdater) notifyWorkers(userID string) {
	_, remoteServer, err := xtools.SplitID('@', userID)
	if err != nil {
		return
	}
	hash := fnv.New32a()
...
func (u *DeviceListUpdater) processServer(serverName spec.ServerName) (time.Duration, bool) {
	...
	logger := xutil.GetLogger(ctx).WithField("server_name", serverName)
...
func (u *DeviceListUpdater) processServerUser(ctx context.Context, serverName spec.ServerName, userID string) (time.Duration, error) {
	...
	if err != nil {
		...
		case xcore.HTTPError:
			...
			// This is to avoid spamming remote servers, which may not be Coddy servers anymore.
			if e.Code >= 300 {
				logger.WithError(e).Debug("GetUserDevices returned xcore.HTTPError")
...
func (u *DeviceListUpdater) updateDeviceList(res *fclient.RespUserDevices) error {
	...
	for i, device := range res.Devices {
		...
		if err != nil {
			xutil.GetLogger(ctx).WithField("keys", device.Keys).Error("failed to marshal keys, skipping device")
			continue
		}
	...
	if err := u.db.DeviceKeysJSON(ctx, existingKeys); err != nil {
		// non-fatal, log and continue
		xutil.GetLogger(ctx).WithError(err).WithField("user_id", res.UserID).Errorf(
			"failed to query device keys json for calculating diffs",
		)
	}
