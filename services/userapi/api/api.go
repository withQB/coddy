...
// custom api functions required by xproto / p2p demos
...
type QueryOpenIDTokenResponse struct {
	Sub         string // The Coddy User ID that generated the token
	ExpiresAtMS int64
}
...
func (d *Device) UserDomain() spec.ServerName {
	_, domain, err := xtools.SplitID('@', d.UserID)
	if err != nil {
...
// Account represents a Coddy account on this home server.
type Account struct {
	UserID       string
	Localpart    string
...
type UserInfo struct {
	Sub string // The Coddy user's ID who generated the token
}
...
type QuerySignaturesRequest struct {
	...
	TargetIDs map[string][]xtools.KeyID `json:"target_ids"`
}
...
type QuerySignaturesResponse struct {
	...
	Signatures map[string]map[xtools.KeyID]types.CrossSigningSigMap
