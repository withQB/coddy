...
func UserIDIsWithinApplicationServiceNamespace(
	...
	var local, domain, err = xtools.SplitID('@', userID)
	if err != nil {
		// Not a valid userID
		return false
	}
...
func UsernameMatchesMultipleExclusiveNamespaces(
	...
	userID := userutil.MakeUserID(username, cfg.Coddy.ServerName)
...
func validateApplicationService(
	...
	if err := internal.ValidateApplicationServiceUsername(username, cfg.Coddy.ServerName); err != nil {
...
func Register(
	...
	if v := cfg.Coddy.VirtualHostForHTTPHost(host); v != nil {
		r.ServerName = v.ServerName
	} else {
		r.ServerName = cfg.Coddy.ServerName
	}
	...
	if r.Username == "" {
		...
		if err = userAPI.QueryNumericLocalpart(req.Context(), nreq, nres); err != nil {
			xutil.GetLogger(req.Context()).WithError(err).Error("userAPI.QueryNumericLocalpart failed")
			return xutil.JSONResponse{
...
func handleGuestRegistration(
	...
	if v := cfg.Coddy.VirtualHost(r.ServerName); v != nil {
...
func handleRegistrationFlow(
	...
	case authtypes.LoginTypeRecaptcha:
		...
		case ErrCaptchaDisabled:
			return xutil.JSONResponse{Code: http.StatusForbidden, JSON: spec.Unknown(err.Error())}
		case ErrMissingResponse:
			return xutil.JSONResponse{Code: http.StatusBadRequest, JSON: spec.BadJSON(err.Error())}
		case ErrInvalidCaptcha:
			return xutil.JSONResponse{Code: http.StatusUnauthorized, JSON: spec.BadJSON(err.Error())}
		case nil:
		default:
			xutil.GetLogger(req.Context()).WithError(err).Error("failed to validate recaptcha")
			return xutil.JSONResponse{Code: http.StatusInternalServerError, JSON: spec.InternalServerError{}}
		}
...
func RegisterAvailable(
	...
	domain := cfg.Coddy.ServerName
	host := spec.ServerName(req.Host)
	if v := cfg.Coddy.VirtualHostForHTTPHost(host); v != nil {
		domain = v.ServerName
	}
	if u, l, err := cfg.Coddy.SplitLocalID('@', username); err == nil {
		username, domain = u, l
	}
	for _, v := range cfg.Coddy.VirtualHosts {
...
func handleSharedSecretRegistration(cfg *config.ClientAPI, userAPI userapi.ClientUserAPI, sr *SharedSecretRegistration, req *http.Request) xutil.JSONResponse {
	...
	if err = internal.ValidateUsername(ssrr.User, cfg.Coddy.ServerName); err != nil {
		return *internal.UsernameResponse(err)
	}
	...
	return completeRegistration(req.Context(), userAPI, ssrr.User, cfg.Coddy.ServerName, ssrr.DisplayName, ssrr.Password, "", req.RemoteAddr, req.UserAgent(), "", false, &ssrr.User, &deviceID, accType)
}
