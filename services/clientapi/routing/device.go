...
func GetDeviceByID(
	...
	if err != nil {
		xutil.GetLogger(req.Context()).WithError(err).Error("QueryDevices failed")
...
func UpdateDeviceByID(
	...
	if err != nil {
		xutil.GetLogger(req.Context()).WithError(err).Error("PerformDeviceUpdate failed")
		return xutil.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
...
func DeleteDeviceById(
	...
	if err := userAPI.PerformDeviceDeletion(ctx, &api.PerformDeviceDeletionRequest{
		...
		xutil.GetLogger(ctx).WithError(err).Error("userAPI.PerformDeviceDeletion failed")
		return xutil.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
...
func DeleteDevices(
	...
	if err = json.Unmarshal(bodyBytes, &payload); err != nil {
		xutil.GetLogger(ctx).WithError(err).Error("unable to unmarshal device deletion request")
		return xutil.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
