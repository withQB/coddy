This component roughly corresponds to "Client Frame Send" and "Client Sync" on [the WIRING diagram](https://github.com/withqb/dendrite/blob/master/WIRING.md).
...
- Each HTTP "write operation" (`/createFrame`, `/frames/$frame_id/send/$type`, etc) is contained entirely to a single file in the `writers` package.
