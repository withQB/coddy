...
type idServerStoreInviteResponse struct {
	...
	PublicKeys  []xtools.PublicKey `json:"public_keys"`
}
...
// matching Coddy ID from the given identity server. If no Coddy ID is
...
// If none of the 3PID-specific fields are supplied, or if a Coddy ID is
...
// fills the Coddy ID in the request body so a normal invite membership event
// can be emitted.
func CheckAndProcessInvite(
	...
	if lookupRes.MXID == "" {
		// No Coddy ID could be found for this 3PID, meaning that a
	...
	// A Coddy ID have been found: set it in the body request and let the process
...
// If the lookup returned a Coddy ID, checks if the current time is within the
...
// If the lookup didn't return a Coddy ID, asks the identity server to store
...
func queryIDServer(
	...
	if lookupRes.MXID == "" {
		// No Coddy ID matches with the given 3PID, ask the server to store the
		// invite and return a token
		storeInviteRes, err = queryIDServerStoreInvite(ctx, userAPI, cfg, device, body, frameID)
		return
	}

	// A Coddy ID matches with the given 3PID
...
func checkIDServerSignatures(
	...
	for keyID := range signatures {
		...
		if err = xtools.VerifyJSON(body.IDServer, xtools.KeyID(keyID), pubKey, marshalledBody); err != nil {
			return err
		}
	}
