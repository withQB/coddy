...
import (
	...
	"github.com/withqb/coddy/services/mediaapi/fileutils"
	"github.com/withqb/coddy/services/mediaapi/storage"
	"github.com/withqb/coddy/services/mediaapi/thumbnailer"
	"github.com/withqb/coddy/services/mediaapi/types"
...
func Upload(req *http.Request, cfg *config.MediaAPI, dev *userapi.Device, db storage.Database, activeThumbnailGeneration *types.ActiveThumbnailGeneration) xutil.JSONResponse {
	...
	return xutil.JSONResponse{
		Code: http.StatusOK,
		JSON: uploadResponse{
			ContentURI: fmt.Sprintf("mxc://%s/%s", cfg.Coddy.ServerName, r.MediaMetadata.MediaID),
		},
	}
}
...
// Returns either an uploadRequest or an error formatted as a xutil.JSONResponse
func parseAndValidateRequest(req *http.Request, cfg *config.MediaAPI, dev *userapi.Device) (*uploadRequest, *xutil.JSONResponse) {
	r := &uploadRequest{
		MediaMetadata: &types.MediaMetadata{
			Origin:        cfg.Coddy.ServerName,
			...
			UserID:        types.CoddyUserID(dev.UserID),
		},
		Logger: xutil.GetLogger(req.Context()).WithField("Origin", cfg.Coddy.ServerName),
	}
...
func requestEntityTooLargeJSONResponse(maxFileSizeBytes config.FileSizeBytes) *xutil.JSONResponse {
	return &xutil.JSONResponse{
...
func (r *uploadRequest) Validate(maxFileSizeBytes config.FileSizeBytes) *xutil.JSONResponse {
	...
	if r.MediaMetadata.UserID != "" {
		...
		// https://github.com/withqb/synapse/blob/v0.19.2/synapse/types.py#L92
		if _, _, err := xtools.SplitID('@', string(r.MediaMetadata.UserID)); err != nil {
			return &xutil.JSONResponse{
				Code: http.StatusBadRequest,
...
// Returns a xutil.JSONResponse error and cleans up directories in case of error.
