...
func (s *serverSigningKeyStatements) BulkSelectServerKeys(
	...
	results := make(map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult, len(requests))
	...
	err := sqlutil.RunLimitedVariablesQuery(
		...
		func(rows *sql.Rows) error {
			for rows.Next() {
				...
				r := xtools.PublicKeyLookupRequest{
					ServerName: spec.ServerName(serverName),
					KeyID:      xtools.KeyID(keyID),
				}
				vk := xtools.VerifyKey{}
				...
				results[r] = xtools.PublicKeyLookupResult{
					VerifyKey:    vk,
...
func (s *serverSigningKeyStatements) UpsertServerKeys(
	...
	key xtools.PublicKeyLookupResult,
) error {
...
func nameAndKeyID(request xtools.PublicKeyLookupRequest) string {
