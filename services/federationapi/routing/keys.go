...
import (
	...
	clienthttputil "github.com/withqb/coddy/services/clientapi/httputil"
...
func localKeys(cfg *config.FederationAPI, serverName spec.ServerName) (*xtools.ServerKeys, error) {
	...
	var err error
	if virtualHost := cfg.Coddy.VirtualHostForHTTPHost(serverName); virtualHost == nil {
		if identity, err = cfg.Coddy.SigningIdentityFor(cfg.Coddy.ServerName); err != nil {
			return nil, err
		}
		publicKey := cfg.Coddy.PrivateKey.Public().(ed25519.PublicKey)
		keys.ServerName = cfg.Coddy.ServerName
		keys.ValidUntilTS = spec.AsTimestamp(time.Now().Add(cfg.Coddy.KeyValidityPeriod))
		...
		for _, oldVerifyKey := range cfg.Coddy.OldVerifyKeys {
		...
		if identity, err = cfg.Coddy.SigningIdentityFor(virtualHost.ServerName); err != nil {
			return nil, err
		}
...
func NotaryKeys(
	...
	for serverName, kidToCriteria := range req.ServerKeys {
		...
		if serverName == cfg.Coddy.ServerName {
		...
		for _, keys := range keyList {
			...
			js, err := xtools.SignJSON(
				string(cfg.Coddy.ServerName), cfg.Coddy.KeyID, cfg.Coddy.PrivateKey, j,
			)
			if err != nil {
