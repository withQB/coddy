...
func MakeJoin(
	...
	createJoinTemplate := func(proto *xtools.ProtoEvent) (xtools.PDU, []xtools.PDU, error) {
		identity, signErr := cfg.Coddy.SigningIdentityFor(request.Destination())
		if signErr != nil {
	...
	input := xtools.HandleMakeJoinInput{
		...
		LocalServerName:   cfg.Coddy.ServerName,
...
func SendJoin(
	...
	input := xtools.HandleSendJoinInput{
		...
		RequestOrigin:     request.Origin(),
		LocalServerName:   cfg.Coddy.ServerName,
		KeyID:             cfg.Coddy.KeyID,
		PrivateKey:        cfg.Coddy.PrivateKey,
		Verifier:          keys,
	...
	// the frame, so set SendAsServer to cfg.Coddy.ServerName
			...
			InputFrameEvents: []api.InputFrameEvent{
				{
					...
					SendAsServer:  string(cfg.Coddy.ServerName),
					TransactionID: nil,
				},
			},
		}, &rsResponse)
