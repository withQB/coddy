...
func (s *FederationInternalAPI) KeyRing() *xtools.KeyRing {
...
func (s *FederationInternalAPI) StoreKeys(
	_ context.Context,
	results map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult,
) error {
...
func (s *FederationInternalAPI) FetchKeys(
	...
	results := map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult{}
	origRequests := map[xtools.PublicKeyLookupRequest]spec.Timestamp{}
...
func (s *FederationInternalAPI) handleLocalKeys(
	...
	for req := range requests {
		if !s.cfg.Coddy.IsLocalServerName(req.ServerName) {
			continue
		}
		if req.KeyID == s.cfg.Coddy.KeyID {
			...
			results[req] = xtools.PublicKeyLookupResult{
				VerifyKey: xtools.VerifyKey{
					Key: spec.Base64Bytes(s.cfg.Coddy.PrivateKey.Public().(ed25519.PublicKey)),
				},
				ExpiredTS:    xtools.PublicKeyNotExpired,
				ValidUntilTS: spec.AsTimestamp(time.Now().Add(s.cfg.Coddy.KeyValidityPeriod)),
			}
		} else {
			// The key request doesn't match our current key. Let's see
			// if it matches any of our old verify keys.
			for _, oldVerifyKey := range s.cfg.Coddy.OldVerifyKeys {
				if req.KeyID == oldVerifyKey.KeyID {
					...
					results[req] = xtools.PublicKeyLookupResult{
						...
						ValidUntilTS: xtools.PublicKeyNotValid,
					}
...
func (s *FederationInternalAPI) handleDatabaseKeys(
	...
	for req, res := range dbResults {
		...
		if res.WasValidAt(now, xtools.StrictValiditySignatureCheck) {
			delete(requests, req)
		}
	}
	return nil
...
func (s *FederationInternalAPI) handleFetcherKeys(
	...
	storeResults := map[xtools.PublicKeyLookupRequest]xtools.PublicKeyLookupResult{}
