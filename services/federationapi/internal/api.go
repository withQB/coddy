...
import (
	...
	"github.com/withqb/coddy/services/federationapi/storage/cache"
...
func NewFederationInternalAPI(
	...
	if keyRing == nil {
		...
		pubKey := cfg.Coddy.PrivateKey.Public().(ed25519.PublicKey)
...
func failBlacklistableError(err error, stats *statistics.ServerStatistics) (until time.Time, blacklisted bool) {
	...
	if mxerr.Code == 401 { // invalid signature in X-Coddy header
		return stats.Failure()
	}
