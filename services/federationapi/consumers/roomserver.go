...
import (
	...
	syncAPITypes "github.com/withqb/coddy/services/syncapi/types"
...
func NewOutputFrameEventConsumer(
	...
	return &OutputFrameEventConsumer{
		...
		queues:        queues,
		rsAPI:         rsAPI,
		durable:       cfg.Coddy.JetStream.Durable("FederationAPIDataFrameConsumer"),
		topic:         cfg.Coddy.JetStream.Prefixed(jetstream.OutputFrameEvent),
		topicPresence: cfg.Coddy.JetStream.Prefixed(jetstream.RequestPresence),
	}
}
...
func (s *OutputFrameEventConsumer) processMessage(ore api.OutputNewFrameEvent, rewritesState bool) error {
	...
	if s.cfg.Coddy.Presence.EnableOutbound && len(addsJoinedHosts) > 0 && ore.Event.Type() == spec.MFrameMember && ore.Event.StateKey() != nil {
...
func (s *OutputFrameEventConsumer) sendPresence(frameID string, addedJoined []types.JoinedHost) {
	...
	edu := &xtools.EDU{
		Type:   spec.MPresence,
		Origin: string(s.cfg.Coddy.ServerName),
	}
	if edu.Content, err = json.Marshal(content); err != nil {
		log.WithError(err).Error("failed to marshal EDU JSON")
		return
	}
	if err := s.queues.SendEDU(edu, s.cfg.Coddy.ServerName, joined); err != nil {
