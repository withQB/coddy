...
func GetState(
	...
	return xutil.JSONResponse{Code: http.StatusOK, JSON: &fclient.RespState{
...
func GetStateIDs(
	...
	return xutil.JSONResponse{Code: http.StatusOK, JSON: fclient.RespStateIDs{
...
func parseEventIDParam(
	request *fclient.FederationRequest,
) (eventID string, resErr *xutil.JSONResponse) {
	URL, err := url.Parse(request.RequestURI())
	if err != nil {
		response := xutil.ErrorResponse(err)
		resErr = &response
		return
	}
