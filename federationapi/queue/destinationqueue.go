...
type destinationQueue struct {
	...
	transactionID      xtools.TransactionID         // last transaction ID if retrying, or "" if last txn was successful
...
// If the queue is empty then it starts a background goroutine to
// start sending events to that destination.
func (oq *destinationQueue) sendEDU(event *xtools.EDU, dbReceipt *receipt.Receipt) {
	if event == nil {
...
// createTransaction generates a xtools.Transaction from the provided pdus and edus.
...
func (oq *destinationQueue) createTransaction(
	pdus []*queuedPDU,
	edus []*queuedEDU,
) (xtools.Transaction, []*receipt.Receipt, []*receipt.Receipt) {
	...
	if oq.transactionID == "" {
		...
		oq.transactionID = xtools.TransactionID(fmt.Sprintf("%d-%d", now, oq.statistics.SuccessCount()))
	}
	oq.transactionIDMutex.Unlock()
